...
Positive: recognized session state survives broad folder selection, custom roots, and aliases without reading its contents. Negative: larger-than-500,000-entry or unreadable trees are retained, and arbitrary exported/renamed transcripts remain outside the recognized-root contract. A measured 460,192-entry cache requires 34.82 seconds for a complete guard walk; the bound is a work limit, not a sampling allowance. Environment overrides must be visible to DiskSage. Concurrent namespace mutation remains a separate hardening concern.
