...
Git reachability proves that a worktree commit is merged into a retained ref, but it cannot prove
that an unmerged pull request was closed. Branch age, a missing upstream, or a deleted remote branch
would be unsupported heuristics. GitHub CLI exposes structured pull-request state and head identity.
...
1. the PR state is exactly `CLOSED`;
...
- Merged worktree cleanup remains available without forge evidence.
