...
The shipped Tauri plan now treats receipt publication readiness as a separate read-only admission projection. It derives the configured `app_data_dir()/receipts/provider-cache` destination and withholds `trash_approval_phrase` unless that parent presently exists as an absolute non-symlink Unix directory at exact mode `0700`. This projection does not grant mutation authority and is intentionally race-tolerant in the safe direction: execution still binds and revalidates the exact parent object immediately before receipt creation, so a later namespace or mode change fails closed. Non-Unix targets continue to receive no actionable Trash approval while the reusable private-publication primitive is unavailable there.
...
The shipped Tauri path derives `app_data_dir()/receipts/provider-cache` but still has no accepted object-bound bootstrap authority for that nested directory. The plan no longer advertises an actionable Trash approval while this prerequisite is absent or mode-drifted; first-use cleanup remains unavailable rather than failing only after the user has supplied confirmation. This removes the false affordance but does not solve provisioning. Until the canonical filesystem owner supplies a cross-platform accepted bootstrap/handle authority or installation lifecycle pre-provisions and proves the exact-private parent, first-use cleanup must remain fail closed rather than mutate without a receipt.
...
- `3b83294c19e6df9017030ff9cb4acd9d6f13aeab`: #303 receipt regression extends the execution contract to reject an existing `0755` receipt parent before cache mutation.
- `0d3fb37f1a82f2b5af4514bb804c11024ace6557`: source-level RED contract requiring the commercial plan projection to withhold Trash approval until the receipt parent is execution-ready; its hosted run was cancelled by the causal repair and is not claimed as terminal RED evidence.
- `2b3aaf020edd21259cd23837c6f5103361685a56`, `dda0b41716023c63e91938d553e51ef8d18df5b2`: commercial readiness projection and shipped Tauri wiring that remove the false actionable approval while retaining exact-object revalidation at execution.

Intermediate RED commits are source/test contract evidence only unless a hosted failing result was actually observed.

## Consequences

DiskSage can surface exact regenerable provider caches and perform reversible Trash cleanup only where receipt/publication and Trash mutation boundaries are actually supported. On Unix, create-new receipt publication is available under a pre-existing exact `0700` parent; the shipped plan now withholds its confirmation phrase while that prerequisite is absent. First-use creation of that parent is still not currently authorized by the reusable owner. Existing-record replacement is deliberately unavailable rather than simulated with a pathname rename. Windows native-handle publication/replacement parity remains a release gap.
