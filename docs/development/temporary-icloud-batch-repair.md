...
The validation pass is intentionally triggered from the ready-for-review state so the temporary fail-closed jobs execute once, prove the regression tests red before the source repair and green afterward, and upload only their digest-verified Rust source artifact.
