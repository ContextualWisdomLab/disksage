...
1. **Equivalence classes (scm-eqc1 + scm-eqc2), by fixpoint**: seed union-find with every explicit `owl:equivalentClass` pair (`scm-eqc1` → bidirectional). Then **iterate to a fixpoint**: build the `subClassOf` graph on the *current* representatives, find its strongly-connected components, merge each SCC>1 into one representative (`scm-eqc2`), and repeat until no SCC>1 remains. The iteration is required because an equivalence merge can *expose* a new representative-level cycle that no pure-`subClassOf` SCC contained — e.g. `A ≡ B`, `B ⊑ C`, `C ⊑ A`: only after `{A,B}` merge do the representative edges `R⊑C`, `C⊑R` form a cycle, collapsing to `A≡B≡C`. Termination: each iteration strictly reduces the representative count, which is bounded. `equivalents()` returns the final group; `cycle_equivalences()` reports groups a merge formed (advisory UI only).
2. **subClassOf closure (scm-sco)**: on the final representative graph — **acyclic** (fixpoint of step 1 leaves no SCC>1) — compute the transitive closure. `scm-cls` makes each class its own distance-0 ancestor/equivalent for lookup.
3. **Class unsatisfiability (cax-dw TBox consequence)** — the sound consequence `C ⊑ owl:Nothing`, in two cases (`disjointWith` is symmetric):
   - (i) for each class `C`, if its ancestor set (incl. self) contains two *distinct* representatives `c1`, `c2` with `c1 owl:disjointWith c2`, emit `UnsatisfiableClass{C, (c1,c2)}`;
   - (ii) if a `disjointWith` pair itself **shares a representative** — equivalent classes declared disjoint, or a self-disjoint class `A owl:disjointWith A` — then that representative is unsatisfiable (`A ⊓ A ⊑ ⊥`), so it *and every class subsumed by it* are emitted. Case (ii) is the corner a naive "two distinct disjoint ancestors" check misses.
   A provably empty class signals the user's ontology is contradictory there.
...
- `disjointWith` unsatisfiability, distinct-ancestor case: `C ⊑ A`, `C ⊑ B`, `A disjointWith B` ⇒ one `UnsatisfiableClass{class:C, ...}`; coherent ontology ⇒ empty.
- `disjointWith` unsatisfiability, **shared-representative corner** (case ii): `A ≡ B`, `A disjointWith B` ⇒ `A` (and its subclasses) unsatisfiable; and self-disjoint `A disjointWith A` ⇒ `A` unsatisfiable. (A naive "two distinct disjoint ancestors" check would miss both.)
...
- **equivalence-fixpoint corner** (mixed `≡`/`⊑` cycle): `A ≡ B`, `B ⊑ C`, `C ⊑ A` ⇒ `equivalents(A)` = {A, B, C} (the merge exposes the second-round SCC), and `check_coherence()` empty. This is the case a single non-iterated SCC pass gets wrong.
...
- **RL subsumption caveat, and why it doesn't apply here.** M. Krötzsch, *The Not-So-Easy Task of Computing Class Subsumptions in OWL RL*, ISWC 2012, LNCS 7649:279–294, shows the OWL 2 RL/RDF rules are **incomplete for TBox class subsumption in general** — that incompleteness arises from the *interaction of complex class expressions* (restrictions, intersection/union), which this design **explicitly excludes** (§2). For the restricted taxonomic fragment here — `subClassOf` + `equivalentClass` + `disjointWith` over named classes — the equivalence-fixpoint + transitive closure is **sound**, and **complete for subsumption among satisfiable classes and for unsatisfiability detection**. (One deliberate materialization gap, not a soundness bug: an unsatisfiable class is entailed to be a subclass of *every* class — ex falso — and `ancestors()` does not materialize those edges; instead such a class is reported by `check_coherence` as `UnsatisfiableClass`, which is the useful signal.) Scoping the fragment is precisely what buys this completeness; extending to restrictions (sub-project A / a future full reasoner) is where a heavier method would be required.
