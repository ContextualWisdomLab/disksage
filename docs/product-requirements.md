...
| FR-3 | Native receipt persistence/readback is covered by `verified_copy_keeps_source_and_writes_read_only_receipt`; the current provider-API fixture covers `prepare_provider_api_source_receipt`, immutable receipt persistence/readback, and `CopiedByProviderApi` identity only. A command-level `provider_api_write::upload_file` flow with a mocked provider response, provider attestation, and permit denial/approval is not yet covered and remains a tracked P1 gap. Bounded copy tests cover pre-start, between-chunk, post-success cancellation, timeout cleanup/retention, private failure-journal restart/readback, materialization-before-hash gating, hash/identity recheck, and eviction-permit denial |
