...
PR #264 is the existing canonical repair. It adds `src/lib/releaseArtifactVerifierDirectoryContract.test.ts`, materializes the complete platform artifact fixture under the real matrix namespaces (`ubuntu-22.04`, `windows-2022`, `macos-latest`), executes the source-controlled verifier, and proves mislocated Windows artifacts fail closed. The production change aligns the verifier's Windows expected directory with `windows-2022` and also binds bundle and operational-CLI discovery to direct placement under each expected platform artifact directory instead of accepting deeper or cross-platform matches. The directory-contract regression rejects nested bundles and verifies the download → shared verifier → SBOM ordering used by tag attestation. Existing gate categories remain intact: exact-set cardinality, checksum, bundle, CLI, non-regular-entry, attestation, and publication checks are still required, while their platform namespace and placement depth are now enforced more precisely.
