...
## Parent-handle evidence and successor integration

Generated-cache owner PR #295 at `b7fbf6f58e196ff7c4b059477af6d450c41373a1` binds the canonical immediate-parent path and device/inode into schema v2. Four nonrecursive parent checks reject active handles, incomplete evidence, and changed identity. Fifteen actual-module tests and two CLI tests pass on macOS. A separate native experiment found the exact live parent-descriptor holder while both target-only and original-command probes reported complete inactivity. This is an additional veto, not exclusion of ancestor, sibling, or later-arriving writers. Atomic no-replace restoration remains a protected prerequisite. Non-Unix planning explicitly rejects unsupported identity; its checked-in regression has not been executed on Windows.

PR #320 at `705c17c09e58ca28b1ee763447aac6f38eaab7a3` preserves its symlink-manifest changes and includes that owner head by normal merge; 17 actual-module tests and two CLI tests pass locally. PR #325 at `7b6d02a13c144d1ed22853709db201bb045a69c0` separately includes the then-current PR #322 head `94de08e29f605521287c65499f340bc83793570d`, with its unique product changes unchanged. The newer #320 head has not yet propagated through #322/#325. All three successors remain Draft while prerequisites are unresolved; no protected merge is claimed.

GitHub job `101514459598` on owner head `b7fbf6f5` completed successfully, but its log executes only the existing Corepack and permanent-Gradle cleanup fixtures. It does not run the generated-cache staged-activity regression. That missing execution belongs to canonical test-workflow PR #338; this runtime lane records the gap without duplicating the workflow. Job success is not coverage of the new parent-handle behavior.

A fresh read-only APFS audit found no Data-volume snapshots. The three system snapshots were OS-update snapshots marked non-purgeable, with no private-allocation figures reported. Native VM observations found the existing Colima and Podman machines running; guest free-space figures are not additional host capacity. Their backing allocations were 46,610,636 KiB (Colima data), 1,834,868 KiB (Colima root), and 2,111,272 KiB (Podman). No VM trim or cache cleanup was performed in this follow-up. At `2026-09-06T15:50:21.055268+00:00`, host available space was 301,337,960 KiB: 278.674946 GiB above baseline and 21.325054 GiB short of the additional-300-GiB target. This is a timestamped host observation, not exclusive operation attribution.
