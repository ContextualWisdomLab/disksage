...
- release concurrency scopes cancellation to pull requests with `github.event_name == 'pull_request' && github.run_attempt == 1`, so a fresh pull-request first attempt supersedes stale work while explicit rerun attempts, tag pushes, and manual runs do not self-cancel through this PR-specific rule;
...
Concurrency cancellation applies only to a first pull-request workflow attempt. A newer first attempt may cancel stale pull-request work for the same PR/ref, but explicit rerun attempts do not cancel themselves because they have `github.run_attempt > 1`. Tag pushes and manual runs are outside this PR-specific cancellation rule. A rerun remains non-authoritative until every required exact-head job in that attempt completes successfully.
