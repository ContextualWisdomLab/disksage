...
- release concurrency never cancels an in-flight build: `cancel-in-progress: false` preserves already-started release work across newer heads and explicit reruns, while Test PR supersession is governed separately by `.github/workflows/test.yml`;
...
Release concurrency never cancels an in-flight build. `cancel-in-progress: false` deliberately lets a started release attempt finish even if a newer head appears or an explicit rerun is requested. Each attempt remains non-authoritative until every required exact-head job for that run completes successfully; completion of an older attempt does not transfer evidence to a newer head.
