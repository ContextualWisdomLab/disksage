...
- all eleven platform-specific operational CLIs and all eleven corresponding `.sha256` files are each present exactly once; the iCloud-named batch planner and cloud-local inventory producer are deliberately macOS-only because their public Foundation metadata contract is unavailable on Linux and Windows;
- the preserved release tree contains exactly 28 regular files (the five desktop bundles, eleven operational CLIs, eleven adjacent checksums, and one source-bound SPDX SBOM) and no symlink, device, socket, FIFO, or other non-regular entry, so unreviewed debug output, logs, dumps, or unrelated executables cannot become attested release subjects;
