...
- all ten platform-specific operational CLIs and all ten corresponding `.sha256` files are each present exactly once; the iCloud-named batch planner is deliberately macOS-only because its public Foundation metadata and eviction contract is unavailable on Linux and Windows;
