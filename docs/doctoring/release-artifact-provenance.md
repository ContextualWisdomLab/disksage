...
- all nine platform-specific operational CLIs and all nine corresponding `.sha256` files are each present exactly once; both batch planners and the cloud-local inventory producer are deliberately macOS-only because supported provider-local observation is not yet implemented on Linux or Windows;
- the preserved release tree contains exactly 24 regular files (the five desktop bundles, nine operational CLIs, nine adjacent checksums, and one source-bound SPDX SBOM) and no symlink, device, socket, FIFO, or other non-regular entry, so unreviewed debug output, logs, dumps, or unrelated executables cannot become attested release subjects;
