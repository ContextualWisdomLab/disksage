...
- release concurrency is repository/PR-scoped: a fresh first-attempt PR run cancels stale work, while an explicit rerun never cancels itself because cancellation is limited to pull-request `github.run_attempt == 1`; draft/closed PR events clean up stale work without starting the build matrix, and `ready_for_review` restarts exact-head release verification;
...
- all ten platform-specific operational CLIs and all ten corresponding `.sha256` files are each present exactly once; both batch planners, the cloud-local inventory producer, and the OneDrive Finder verifier are deliberately macOS-only because supported provider-local observation is not yet implemented on Linux or Windows;
- the preserved release tree contains exactly 26 regular files (the five desktop bundles, ten operational CLIs, ten adjacent checksums, and one source-bound SPDX SBOM) and no symlink, device, socket, FIFO, or other non-regular entry, so unreviewed debug output, logs, dumps, or unrelated executables cannot become attested release subjects;
...
## Customer and operator verification
