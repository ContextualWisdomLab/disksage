...
| P0 | Independent exact-head test runs exhaust the GitHub-hosted Ubuntu disk while linking repeated feature profiles. | On 2026-08-29, PRs #286, #296, and #297 reached OS error 28 while creating `libdisksage_lib.a`; earlier test output was green. PR #298 combines the cloud/archive feature suite into one Cargo invocation, disables unused incremental/debug artifacts, and reclaims only official image SDK roots outside the job capability set with exact before/after byte evidence. Its first exact-head run proved the disk repair effective but reached the former 30-minute job limit after the base Rust suite completed in 17m33s and the combined feature suite had run for 11m36s, before frontend verification. The bounded job budget is therefore 60 minutes so both independently required Rust configurations and the frontend gates can finish without splitting and recompiling the feature profile. | The #298 test job completes on its exact head and publishes positive reclaimed-byte evidence; descendants inherit the workflow before their checks are treated as release evidence. |
...
## 2026-08-29 OneDrive local-copy eviction evidence

- PR #296 head `3f7f0f61a4f3a92d96b2431a7d50f59b6fab96c5`, workflow run
  `33239956555`, and artifact `9711163544` produced the packaged macOS batch CLI. The downloaded
  artifact digest `e99d12ec0e8711962e0e9468daf3b513b647dbaf1b8394690c37d1b4e882b89c`
  matched GitHub metadata and every adjacent executable checksum passed.
- A freshly rediscovered readable OneDrive personal root was bound by the path-free identity digest
