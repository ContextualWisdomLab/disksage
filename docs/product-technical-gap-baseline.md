...
| Standalone Git clones | Move a clean, inactive, single-worktree clone whose local HEAD is contained by the exact head of a closed or merged same-repository PR to OS Trash | Bounded multi-root discovery; fresh completed/open PR membership; exact authoritative PR head ancestry; fresh default-branch identity; recursive active-use check; owner-created durable lease evidence; internal Git directory; object identity; exact approval | Default branches, open or stale-open PRs, active or invalid checkout leases, branch deletion, `git prune`, stale refs, unique/unpushed/diverged/fork/detached/dirty/active clones, dataless provider items, external Git directories, or implicit age and lease thresholds |
