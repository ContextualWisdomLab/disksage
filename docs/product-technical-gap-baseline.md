...
| P1 | Large stopped Parallels VMs lacked a trustworthy reclaim estimate. | One local observation found about 103 GiB under `~/Parallels`, while native compact-info evidence estimated only about 48 MiB reclaimable; directory size is therefore not a reclaim claim. | A macOS-only read-only plan binds stopped state, canonical bundle/disk identity, bounded allocation and active-use evidence, and the native estimate; mutation remains unavailable until a separately reviewed execution boundary exists. |
