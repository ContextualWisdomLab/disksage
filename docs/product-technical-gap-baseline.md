...
| P0 | Independent exact-head test runs exhaust the GitHub-hosted Ubuntu disk while linking repeated feature profiles. | On 2026-08-29, PRs #286, #296, and #297 reached OS error 28 while creating `libdisksage_lib.a`; earlier test output was green. PR #298 combines the cloud/archive feature suite into one Cargo invocation, disables unused incremental/debug artifacts, and reclaims only official image SDK roots outside the job capability set with exact before/after byte evidence. | The #298 test job completes on its exact head and publishes positive reclaimed-byte evidence; descendants inherit the workflow before their checks are treated as release evidence. |
