...
| #311 | `258b57f315b4d0470dc19318371b5534fd22fee7` | `feat/native-uv-cache-reclaim-v1` | ready, unstable | Plan and Trash standalone clones only with closed/merged exact authority; current checks include a failed test and running builds, with three unresolved threads and no exact-head approval |
...
| #316 | `465ad0816d99dd3759c7ecb69aabf6499caad277` | `feat/podman-storage-repair-v1` | ready, unstable | Review exact photo duplicates, choose an unambiguous keeper, and use reversible quarantine; current checks include failed test and Windows build jobs, with seven unresolved threads and no exact-head approval |
...
| --- | --- | --- |
| APFS global sample, 2026-08-30 06:14:22 KST | Data volume `804,124,128 KiB` used and `104,161,616 KiB` available (`106.66 GB`, `99.34 GiB`) | This shared-volume sample fluctuates with concurrent activity and is not reclaim attribution. The 300 GB product goal remains unmet; re-sample immediately before and after each approved action. |
...
| P0 | Photo quality-aware duplicate cleanup is not integrated. | #313 provides exact decoded-pixel PNG evidence, while stacked #316 adds customer review and reversible quarantine. Both remain outside protected `main`; #316 has failed checks, unresolved review threads, and no exact-head approval. Perceptual near-duplicate grouping and calibrated no-reference IQA remain unavailable. | Review the exact duplicate group, choose a keeper when evidence ties, approve the displayed identities, and verify the restore receipt before counting recovery. |
