...
| P1 | Temporary storage can be large, but generic age cleanup cannot distinguish active or private data. | The native temp planner now performs a bounded two-level discovery and admits only independently marked generated build roots with complete manifests and handle evidence. Unknown entries remain unavailable. | Extend only through a new evidence-backed candidate adapter; retain exact approval, identity recheck, Trash journal, and permanent-delete prohibition. |
