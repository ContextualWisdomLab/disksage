...
| P1 | Large stopped Parallels VMs lacked a trustworthy reclaim and execution boundary. | One local observation found about 103 GiB under `~/Parallels`, while native compact-info evidence estimated only about 48 MiB reclaimable; directory size is therefore not a reclaim claim. | PR #312 now plans and executes only a fresh, exact-approved native compact on a stopped registered VM with no snapshots, complete inactive-use evidence, unchanged bundle/disk identity and allocation, fixed vendor paths, immutable approval/result records, and before/after physical evidence. It never uses `--force`; the observed result, not bundle size or estimate, is the recovery claim. |
