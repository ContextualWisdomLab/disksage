...
**Repository heads at snapshot:** PR #262 `fbc442d5`, PR #260 `041cce6`, PR #259 `ae5e158`, PR #258 `f3a25b3`, PR #257 `d09808f`, PR #253 `7db00e2`, PR #251 `7b8caa3`, PR #227 `98fa709`, PR #249 `7c93e17`,
...
| #262 | `fbc442d53aff31b792a6ea3db60766b3951a5f54` | `7eb131c4` | no | blocked | required | private failure records are isolated from success receipts and capped at 10,000 entries; native cancellation is bound to the active candidate and no longer appears for provider-API uploads; production adoption always probes provider-native local-current status before and after final identity checks; failed native-copy cleanup is identity-bound on Unix and Windows; focused Rust contract tests pass 4/4, while current hosted checks and independent approval remain authoritative |
...
| #258 | `f3a25b3ed88af7479174ea3c6d3d6771c75a22e9` | `7eb131c4` | no | blocked | required | current baseline/docs head before the #262 exact-head follow-up; records PR #251/#253 dependency evidence and the current timeout, privacy-scope, cancellation, and placeholder-adoption gaps; hosted checks and qualifying approval remain authoritative |
...
| P1 | A user cannot cancel an in-flight DiskSage native copy from the UI; Finder cancellation is a separate external action. | `copyCandidate` awaits the IPC operation and only exposes a busy label; the backend copy loop has no receipt-bound cancellation command. | A receipt-bound cancel action stops the bounded copy at a safe chunk boundary, removes only the owned destination, records cancellation durably, and leaves source/eviction untouched. |
...
| P1 | Copy cancellation is not represented in the operation state machine. | UI busy state has no cancel transition; backend has no operation identifier or cancellation token for `copy_cloud_candidate`. | Add one serialized copy operation per app instance, an explicit cancel command, and tests for cancel-before-start, cancel-between-chunks, and cancel-after-success. |
...
- DiskSage keeps new copy, provider attestation, and source eviction fail-closed. It does not kill
  ...
  #259 (`0554671ad0f42a29f0fd66e21f93d6d2f5b780e7`) exposes no-progress labels, bounded upload/
