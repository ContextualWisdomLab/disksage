...
| P1 | Colima backing disks can retain host allocation after container cleanup, but unsafe raw-disk advice risks VM corruption. | A bounded Rust plan now reports profile/runtime state, logical and physical allocation, and backing-disk identity. Colima currently documents only running-guest `fstrim`; no native stopped-VM compact command exists. | Keep execution unavailable with a truthful receipt until Colima publishes a supported native stopped-VM compact command; never stop the VM or invoke `qemu-img` on the user's behalf. |
