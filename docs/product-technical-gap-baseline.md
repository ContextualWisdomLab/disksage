...
Review repair `6ea7992e` adds a second session check after native Git staging and restores rejected worktrees without overwriting reappeared paths. Failed cloud Trash callbacks restore verified regular files when the original path is available. Permanent cache deletion now fails closed using PR #263's policy; that PR's remaining approval/provenance deltas are not superseded. The first full library run reported 757 passed, 4 failed, and 1 ignored: one new macOS path-alias assertion was corrected in `95ceeb21`; three live active-use observation failures require separate evidence. Hosted workflow failures remain owned by PR #264 and central review/security workflows. No exact-head green or protected merge is claimed.

The user raised P1 acceptance to a verified 300 GiB increase in actual available capacity without session false positives. Native online free-block return in Colima and Podman reduced backing allocation by 39.984161 GiB without deleting files, images, or containers. The measured host increase from the initial 8.70 GiB available baseline was 39.104523 GiB, leaving 260.895477 GiB unmet. PR #310 remains the owner of the separate stopped-VM planning contract; no unavailable product execution gate was changed or successful product receipt invented. The [operation evidence](doctoring/session-preservation/README.md#capacity-phase-online-free-block-return) distinguishes guest discard counts, backing allocation, host free space, and concurrent activity. Large iCloud and temporary-directory inventories remain incomplete; they provide no deletion authority.
