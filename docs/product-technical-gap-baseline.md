...
| #312 | `880a76460527e9c93144acf4eb4eecc38bec7725` | `fix/release-artifact-namespace-v2` | ready, unstable | Add read-only, fail-closed planning for stopped Parallels VM disk reclaim and publish the planner on macOS; current checks include failed review/security gates and running test/build jobs, with six unresolved threads and no exact-head approval |
...
| --- | --- | --- |
| APFS global sample, 2026-08-30 06:00:10 KST | Data volume `801,174,916 KiB` used and `108,159,504 KiB` available (`110.76 GB`, `103.15 GiB`) | This shared-volume sample fluctuates with concurrent activity and is not reclaim attribution. The 300 GB product goal remains unmet; re-sample immediately before and after each approved action. |
