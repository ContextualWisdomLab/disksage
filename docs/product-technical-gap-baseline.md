...
| Shared temporary storage (`/tmp` or macOS `/private/tmp`) | Show current-user-owned children; permanently reclaim only top-level DiskSage artifacts carrying exact producer completion evidence | Real-directory root, sealed object identity, complete bounded tree fingerprint, ownership and active-use evidence, no links/sockets/locks/worktree/database data, exact approval, fresh revalidation, deletion journal, immutable receipt, and native available-byte observations | Age/name-only authority, other-user/system-owned trees, symlinks, active references, database/worktree data, and deleting the shared root itself |
