...
`c9e7ebd8fb403d7420c073c762e4203ba7397ca6` (queued-copy cancellation is
checked throughout native preflight, cancellation is journaled, and the
operation reset is panic-safe; focused authority/cancellation contract tests
pass; review threads are resolved; hosted checks and approval remain
required). Naruon PR #1469 is now
`258ac418e77afef7d725705294cb8a0a537db6b2` (expected payload-size rejection is
INFO telemetry, ADR-0005 now distinguishes the current 20MiB direct DOM
contract from the proposed 64MiB alignment, focused worker/client/parser tests
pass 54/54; hosted checks and fresh review remain required). Central
...
head is `4232668e88a73bc2c766b986605fe7915c3474df`; its table row intentionally
records the immediately preceding committed snapshot.
DiskSage PR #198 is now
`c1281fb2dcb3126ec3f5e6952865e79a28e2a1f4`; the unused UI blocker helpers are
removed, native-status-unavailable has bounded customer guidance, ineligible
...
| #262 | `c9e7ebd8fb403d7420c073c762e4203ba7397ca6` | `7eb131c4` | no | blocked | required | private failure records are isolated from success receipts and capped at 10,000 entries; same-millisecond duplicate failures now receive bounded suffix IDs instead of being dropped; native cancellation is bound to the active candidate and no longer appears for provider-API uploads; adoption is not registered as a cancellable native copy; queued native copies register before the shared review lock so early cancellation is retained, duplicate native copies cannot overwrite the active token, and the UI recognizes bounded cancellation-plus-journal-error outcomes; cancellation is checked throughout native preflight and journaled, native operation reset is panic-safe, and Windows metadata-only adoption remains fail-closed; focused authority/cancellation contract tests pass 7/7, unresolved review threads are zero, and fresh hosted checks and independent approval remain authoritative |
| #260 | `9cebf0bf0382d3cdc50a030f388729acf46fba58` | `7eb131c4` | no | blocked | required | current-main successor for closed draft #225; Rust planner rejects cwd-dependent, named-tilde, and parent-traversal ontology targets, preserves literal absolute tildes, keeps Windows contract tests inside the crate, accepts native `~\\` home tokens, and scopes the Windows job to `organize::tests::windows_`; local organize suite is 22 passed and hosted `windows-home-resolution` is SUCCESS (job `97834797298`), with remaining checks/approval authoritative |
| #258 | `3778703c2093aecd9c60903bbfbd0ec50efc9ffb` | `7eb131c4` | no | blocked | required | current baseline/docs head; records PR #251/#253 dependency evidence plus metadata precedence, private failure journal, provider-native materialization gate, cancellation, provider-managed traversal, placeholder-adoption gaps, and the path-bearing local Naruon compatibility-envelope exception; hosted checks and qualifying approval remain authoritative |
...
| #246 | `57eb6c134a3a29fe231d973c35df762edd9fa34d` | `7eb131c4` | no | blocked | required | current skip-link contrast contract; the prior iCloud stall-clock and non-iCloud notice fixes remain in the exact-head history; hosted checks and approval remain authoritative |
| #227 | `4a41bb8dd4739763c0fda579d1a20cf938f3e994` | `7eb131c4` | no | blocked | required | duplicate, multipart, incomplete-download, recovery, and materialization reads use descriptor-relative `BoundReadRoot` traversal; multipart audit now parses names before opening metadata so unrelated unreadable files are skipped; targeted unreadable-unrelated test passes 14/14, review threads are resolved, and hosted checks plus approval remain authoritative |
...
| #217 | `687c0148e608fcf6ffe4bc0e13a2f9c2d88ffbde` | `7eb131c4` | no | blocked | required | eviction destination CLI help is bounded and successful for sole help/invalid UTF-8; review threads are resolved, current Strix failure is provider rate-limit/backend evidence, and protected checks/approval remain authoritative |
| #216 | `043a756f70356113c81c78f731f1b36f384943c3` | `7eb131c4` | no | blocked | required | provider runtime CLI help contract; current test rerun is pending after a prior cancellation, while Strix passed and other terminal checks passed; protected checks/approval remain authoritative |
| #214 | `ce88169954e478b6335cfd7971db819cb828edb1` | `7eb131c4` | no | blocked | required | Maven cache CLI help contract with private output boundary; review threads are resolved, current Strix failure is provider rate-limit/backend evidence, and protected checks/approval remain authoritative |
| #198 | `c1281fb2dcb3126ec3f5e6952865e79a28e2a1f4` | `7eb131c4` | no | blocked | required | iCloud eviction privacy feedback hardening; unused blocker helpers removed, native-status-unavailable guidance is explicit, ineligible plans filter approval guidance, focused UI checks pass 8/8, review threads are resolved, hosted checks and qualifying approval remain authoritative |
...
PR #1469 is `258ac418e77afef7d725705294cb8a0a537db6b2`; its 64 MiB deferred attachment retention now
...
- Naruon PR #1469 is currently `258ac418e77afef7d725705294cb8a0a537db6b2`. Required Strix and image
...
- DiskSage PR #262 is currently exact head `c9e7ebd8fb403d7420c073c762e4203ba7397ca6`. The
  cancellation/journal/RAII fix is pushed, current review threads are zero, and protected auto-merge
  is enabled. Hosted checks are still running; `REVIEW_REQUIRED`/`BLOCKED` remains authoritative and
  no merge is claimed.
...
- Central `ContextualWisdomLab/.github` PR #1331 is `40955bab7663c86bf642995b417cb9c560cb0778` and
  remains open with a blocked protected state while fresh checks run; #1333 is `5454a19618b27f4d7f34dd2a7a5e47eb5bc42fc1` and
  merged on 2026-08-25. These exact heads supersede the older abbreviated table entries above.
...
- DiskSage PR #208 advanced to exact head `e5fcc1653a667b0291851568ab038493828e73b6` after
  ...
  is claimed.
- DiskSage PR #262 is now exact head `c9e7ebd8fb403d7420c073c762e4203ba7397ca6` and remains
  ...
  cancellation checks, surfaced failure-journal errors, queued-copy cancellation registration,
  preflight cancellation journaling, and panic-safe operation cleanup, and
  Windows placeholder fail-closed adoption; focused cloud-transfer 25/25, provider-sync 21/21
  (1 ignored), and authority/cancellation contract 7/7 suites pass,
  with zero unresolved review threads and no current terminal Check failure observed.
...
- The documentation PR #258 is now exact head `3778703c2093aecd9c60903bbfbd0ec50efc9ffb`; this
  section is the current reconciliation for the protected review/check gate.
- DiskSage PR #246's predecessor `1ae5a3517f80aeda79c3e77fe540ccb2ce1f4ccb` carried the
  non-iCloud provider checking-state card; the current exact head is `57eb6c134a3a29fe231d973c35df762edd9fa34d`.
...
| P1 | Provider-API command flow lacks deterministic end-to-end receipt and evidence coverage. | The current fixture exercises `prepare_provider_api_source_receipt` plus immutable receipt readback and `CopiedByProviderApi` identity, but it does not invoke `commands::copy_cloud_candidate_via_provider_api` or `provider_api_write::upload_file`; fixed external endpoints and credential lookup make that path non-deterministic in tests. | Add an injectable provider transport/credential seam, then run a mock command-level upload that reads the receipt back and proves permit denial before valid evidence and approval after valid evidence. |
...
| P2 | Public and private lineage privacy scopes are not named consistently. | Local receipt/UI schemas and the compatibility `NaruonFileLineageEnvelope` intentionally retain source/destination paths for recovery; only the redacted Naruon/public readiness projection is shareable and path-free. | Keep the compatibility envelope inside the local trust boundary and require the redacted projection for exports; mirror the distinction in ADR, API types, and export tests. |
...
| #246 | `9d82358cce5e4f56430b083bcfc58aed5323594b` | blocked, review required, auto-merge enabled | Customer guidance now hides implementation terms in iCloud feedback, copy records, page metadata, capacity status, and orphan cleanup; the executable contract rejects forbidden vocabulary and requires next actions | Full UI suite 173/173, Svelte check 0 errors/0 warnings, and diff check pass; hosted checks and independent approval remain required |
...
| #258 | `fcc3eef684b188e46f0ae28407bd6ee5d5012ded` | blocked, review required, auto-merge enabled | This baseline and PRD capture the live product gaps, current PR evidence, and the customer-copy contract; the row intentionally records the latest committed predecessor because this file is the PR itself | Documentation diff check passes; hosted checks and independent approval remain required |
| #266 | `df8d0f1dd103fdf83f5d738ad3b46fc2dca096a6` | blocked, review required, auto-merge enabled | Ancestor inventory scans prune only verified provider roots under the user home; explicit provider-root scans remain available, nested same-named folders are preserved, and symlinked home scans retain protection | Scanner suite 20/20 passes; hosted checks and independent approval remain required |
| #257 | `b13d36527c0a1241e93337326495712b87588367` | blocked, review required, auto-merge enabled | Svelte dependency update with no customer-copy change | Strix is the only observed terminal failure and is covered by the central provider-compatibility repair; other checks remain authoritative |
| #256 | `cc6db7ad6d0dd7716d099392f5239049027c6166` | blocked, review required, auto-merge enabled | Vite dependency update with no customer-copy change | Strix is the only observed terminal failure and is covered by the central provider-compatibility repair; other checks remain authoritative |
| #255 | `69ca78167c0d938445177c5fdfa235dcc5f3ff51` | blocked, review required, auto-merge enabled | SvelteKit dependency update with no customer-copy change | Strix is the only observed terminal failure and is covered by the central provider-compatibility repair; other checks remain authoritative |
| `.github #1352` | `5331cb18ff3a3ddd657bd74c48f357f20cb009d0` | blocked, checks queued, auto-merge enabled | Central Strix runner selects reasoning effort per child model so direct OpenAI fallbacks receive the provider-compatible neutral value | Targeted contract suite 51/51 and shell syntax/diff checks pass; hosted checks and independent approval remain required |
