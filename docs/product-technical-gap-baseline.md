...
6. Provider cache reclaim is exact and class-specific: a content-addressed Podman AppleHV seed, a superseded Edge installed copy, and EdgeUpdater CRX cache are separate candidates. Planning is dry-run; Trash and permanent purge are separate approvals, and active VM disks are excluded.
...
| P0 | Regenerable provider caches consumed physical space but had no exact permanent-reclaim path. | The Rust planner now binds Edge installed/cached versions, AppleHV machine/active-disk/recreation evidence, full fingerprints, and open-handle evidence. Trash and permanent purge are distinct; permanent purge requires double fingerprint confirmation and a create-only receipt. A redacted live operation reclaimed 1,792,412 KiB APFS from 1,797,688 KiB admitted allocation. | Keep exact-candidate tests and hosted checks green; expose the dry-run and explicit backend-authored confirmation in the product UI without internal implementation language. |
| P0 | A damaged Podman layer can make `podman system df` and native repair fail while volumes remain ambiguous. | The assessor emits `repair-required` and only recommends read-only `podman system check --quick`; it does not auto-repair or blanket-prune. Guest TRIM output is not treated as host reclaim proof. | Add a separately reviewed lineage repair design before any mutation; continue preserving running containers and all volumes without exact ownership/recreation evidence. |
...
| P1 | Self-updating developer tools can retain superseded binaries outside known provider caches. | A redacted observation retained the exact binary selected by the tool's active symlink while two non-target downloads had no open handles; removing only those exact non-targets increased APFS availability by 222,712 KiB from 232,052 KiB admitted allocation. | Generalize only after a signed updater/active-symlink contract is identified: retain the exact resolved target and require fingerprint, no-open-handle evidence, double confirmation, and a receipt for every non-target binary. |
...
- ADR-0012 defines exact provider-cache admission, separate Trash/permanent modes, immutable receipts, and fail-closed Podman store diagnosis.
