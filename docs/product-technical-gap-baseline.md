...
**Repository heads at snapshot:** PR #262 `65f06d9`, PR #260 `041cce6`, PR #259 `ae5e158`, PR #258 `0931af7`, PR #257 `d09808f`, PR #253 `7db00e2`, PR #251 `7b8caa3`, PR #227 `98fa709`, PR #249 `7c93e17`,
...
| #262 | `65f06d9f1c8b8173070180537060c1f15690057f` | `7eb131c4` | no | blocked | required | private failure records are isolated from success receipts and capped at 10,000 entries; native cancellation is bound to the active candidate and no longer appears for provider-API uploads; the UI exposes cancellation only for the cancellable native path (frontend contract test added); coverage cfgs now compile successfully under `RUSTFLAGS=--cfg coverage`; production adoption always probes provider-native local-current status before and after final identity checks; failed native-copy cleanup is identity-bound on Unix and Windows, while adoption is explicitly non-cancellable; cancellation feedback is now bounded through the same redaction contract and its privacy test enumerates the new operation; focused Rust contract tests pass 4/4, unresolved review threads are zero, and current hosted checks and independent approval remain authoritative |
