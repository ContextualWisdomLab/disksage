...
| PR #203 — TopFiles accessibility | `fce4b1b92f4493063ce1c48ddc3e9929732731f4` | stacked on exact #202; TopFiles component/accessibility owner | Draft, mechanically mergeable, `behind_by=0`; semantic delta is exactly `TopFiles.svelte` plus its focused test. SSR now validates rendered accessibility structure. One browser-interaction review thread remains open because fragment activation/focus is not executed in a browser harness. | Add or consume a canonical browser/E2E contract that proves keyboard fragment activation and focus/scroll behavior on the unchanged head; then reacquire gates/review. |
...
| P1 | TopFiles had a buyer-visible empty-state and keyboard-evidence gap. | #202 now makes zero-result guidance reachable after a successful scan; #203 validates rendered heading/table/header/region/status structure with Svelte SSR. The repository still has no executing browser DOM/Playwright/Tauri-driver contract that proves fragment activation moves focus into the scroll region. | Current-head browser E2E exercises non-empty and empty states, keyboard activation of the fragment link, resulting focus/scroll target, visible focus, and no regression in initial/loading/error rendering. The corresponding review thread is resolved only after that evidence exists. |
