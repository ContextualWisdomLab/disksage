...
| P1 | Open PR queue prevents a clean protected release line. | PR #213 is at exact head `4200b62`; its required checks remain queued/in progress and the review decision is still a stale `CHANGES_REQUESTED`. PR #209 now carries head `aa3039d` with a privacy-safe iCloud audit alert and is under fresh checks. PR #235 was protected-squash merged as `5f7c7ae`. | Process one PR at a time: current-head review → fix → required checks → fresh approval → normal protected merge; never bypass or self-approve. |
