...
| #311 | `e1a36b3c13e526524bca92eec2f5d30d77125936` | `feat/native-uv-cache-reclaim-v1` | ready, unstable | Plan and Trash standalone clones only with closed/merged exact authority after merging the latest #285 base and repairing the safety-facade verifier export; focused Rust suites passed 15/15, the LLM feature compiled, and Gradle fixtures passed 4/4, while hosted checks are running, one older actionable thread awaits current-head disposition, and no exact-head approval exists |
...
| #316 | `a8a5dfe3e9664de58446db1a722174a9317ac1d9` | `feat/podman-storage-repair-v1` | ready, unstable | Review exact photo duplicates, choose an unambiguous keeper, and use reversible quarantine; the stack includes Linux approval fixtures, fail-closed Windows compilation, native active-use/path handling, chronological receipts, multi-encoding comparison, and terminal help, with focused Rust 23, Vitest 16, and zero-warning Svelte validation reported before the final help commits; hosted checks are rerunning, two new customer-flow findings remain unresolved, and no exact-head approval exists |
...
| --- | --- | --- |
| APFS global sample, 2026-08-30 06:19:20 KST | Data volume `797,356,244 KiB` used and `111,978,176 KiB` available (`114.67 GB`, `106.79 GiB`) | This shared-volume sample fluctuates with concurrent activity and is not reclaim attribution. The 300 GB product goal remains unmet; keep it separate from the action-attributable cleanup rows and re-sample immediately before and after each approved action. |
...
| Current iCloud native revalidation, 2026-08-30 | The bounded revalidation did not produce a complete item-specific native snapshot; the latest retained complete redacted audit remains `needs-sync`, with 120 items and 0 eligible local-copy evictions | Keep the local source. Let iCloud finish, then refresh the complete native inventory; `local-current` with `is_uploaded=false`, incomplete, or timed-out evidence cannot issue an eviction permit. |
...
| P0 | Photo quality-aware duplicate cleanup is not integrated. | #313 provides exact decoded-pixel PNG evidence, while stacked #316 adds customer review and reversible quarantine. Both remain outside protected `main`; current #316 still has two unresolved customer-flow findings: a repeat scan may retain obsolete selections, and files chosen outside the scanned folder cannot complete the recovery plan. Perceptual near-duplicate grouping and calibrated no-reference IQA remain unavailable. | Scan again after the selection flow is repaired, keep every chosen file inside the displayed recovery scope, choose a keeper when evidence ties, and verify the restore receipt before counting recovery. |
