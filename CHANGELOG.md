...
- Bind the default on-device GGUF model to an immutable upstream revision, exact byte count, and SHA-256 digest; replace whole-model memory buffering with bounded streaming into a create-new staging file; refuse destination/staging overwrite; finalize with no-clobber same-directory linking only after exact-size, digest, flush, and file-sync verification; bind pathname cleanup to the verified file identity so concurrent staging or destination replacements are preserved rather than deleted; and keep model installation inside the Rust coverage surface with privacy-safe stable errors.
