...
- Enable an explicit fail-closed Tauri Content Security Policy that keeps executable scripts and fonts local, grants network authority only to the Tauri IPC transport, denies object/frame/base-URI authority, and regression-tests against null, wildcard, remote-script/style, and eval policy drift.
...
- Generate the npm lockfile in an exact-head validation job with repository data read-only and dependency lifecycle scripts disabled, bind the artifact to SHA-256 evidence, and grant repository modification permission only to a separate publication job that verifies the same-run artifact and unchanged branch head before publishing the lockfile update.
- Removed obsolete recovery automation and patch helpers so repository automation no longer retains dormant branch-modification paths.
