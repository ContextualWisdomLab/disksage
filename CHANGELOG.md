...
- Bind the default on-device GGUF model to an immutable upstream revision, exact byte count, and SHA-256 digest; replace whole-model memory buffering with bounded streaming into a create-new staging file; refuse destination/staging overwrite; finalize with no-clobber same-directory linking only after exact-size, digest, flush, and file-sync verification; preserve concurrent staging or destination pathname replacements while removing a failed destination link only when it still has the identity captured immediately after DiskSage created it; and keep model installation inside the Rust coverage surface with privacy-safe stable errors.
