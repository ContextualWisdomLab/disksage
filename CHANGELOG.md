...
- Bind the default on-device GGUF model to an immutable upstream revision, exact byte count, and SHA-256 digest; replace whole-model memory buffering with bounded streaming into a create-new staging file; refuse destination/staging overwrite; capture destination ownership directly from a create-new open file handle instead of inferring ownership after a hard-link race; reread and rehash the still-open verified staging file during final copy so same-inode mutation, growth, and truncation fail closed; preserve concurrent staging or destination pathname replacements through identity-bound cleanup; and keep model installation inside the Rust coverage surface with privacy-safe stable errors.
