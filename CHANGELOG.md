...
- Replace generator-era Cargo package metadata with the DiskSage product description, Contextual Wisdom Lab attribution, MIT license expression, source repository URL, and `publish = false` registry-publication boundary, with a deterministic acquisition-metadata regression contract and doctoring evidence.
