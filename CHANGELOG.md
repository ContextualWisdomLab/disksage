...
## [Unreleased]
- Add a read-only, caller-budgeted allocation map that reports incomplete evidence explicitly,
  never follows symbolic links or crosses devices, and classifies provider, Photos, VM, cache,
  generated, per-user system-cache, and customer-data boundaries without granting deletion.
