...
for required_name in \
  ...
  disksage-cloud-local-inventory-macos-arm64; do
...
if [[ ${#checksum_files[@]} -ne 11 ]]; then
  printf 'Expected eleven operational CLI checksum files, found %s.\n' "${#checksum_files[@]}" >&2
  exit 1
fi
...
if [[ $regular_file_count -ne 27 ]]; then
  printf 'Unexpected release artifact entries: expected exactly 27 regular files, found %s.\n' "$regular_file_count" >&2
  exit 1
fi
