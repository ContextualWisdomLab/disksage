...
- ⏸️ **iCloud pre-copy pressure gate** — reads the private CloudDocs database through immutable SQLite mode, emits only queue aggregates and stable blocker codes, and fails closed before a new iCloud copy when the existing local upload queue is non-empty or unhealthy; a quiet queue still does not prove remote capacity, per-item synchronization, or eviction safety
- 🧾 **Naruon cloud-copy readiness envelope** — combines path-free production-time evidence aggregates, planner/review blockers, provider-client runtime, authoritative capacity assessment, and the optional iCloud admission gate; binds them with a recursively key-sorted SHA-256 fingerprint while keeping every write, sync, review, and eviction authority false
- 🔒 **Private full-plan review dossier** — writes every candidate in one fresh single-destination plan, including blocked items and raw metadata evidence, to a bounded create-new mode-0600 file while stdout exposes only path-free aggregates and an integrity checksum
- 📦 **Archive inclusion batch audit** — streams each bounded ZIP once, proves exact internal path/mode/size/SHA-256 inclusion across feasible pairs, emits only content identities to Naruon, and keeps local paths in an optional create-new mode-0600 private report without authorizing deletion
