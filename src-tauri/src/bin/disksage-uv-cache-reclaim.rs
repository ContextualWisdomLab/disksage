...
const USAGE: &str = "Usage: disksage-uv-cache-reclaim [--uv-bin PATH] [--execute --approved-plan-fingerprint HEX --confirm PHRASE --approved-by human:ID --rationale TEXT --record-dir PATH]\nWithout --execute, prints a read-only native uv cache prune plan.";
