...
fn run() -> Result<(), String> {
    let raw = std::env::args_os()
        ...
        .map(|argument| {
            argument.into_string().map_err(|_| {
                "incomplete-download-materialization-unknown-argument".to_string()
            })
        })
    ...
    let report =
        plan_incomplete_download_materialization(&args.root, &audit, &recovery, system_now_ms())?;
    let mut summary = serde_json::to_value(summarize_incomplete_download_materialization(&report))
    ...
    if let Some(path) = &args.private_output {
        ...
        summary
            .as_object_mut()
            .ok_or_else(|| {
                "incomplete download materialization summary JSON object가 아님".to_string()
            })?
            .insert(
                "private_output".into(),
...
mod tests {
    ...
    fn parses_bounded_absolute_arguments() {
        let args = parse_args(&[
            ...
            "60".into(),
            "--private-output".into(),
            "/private/materialization.json".into(),
        ])
        .unwrap();
        ...
        assert_eq!(
            args.private_output,
            Some(PathBuf::from("/private/materialization.json"))
        );
    }
