...
pub fn merge_insight(ext: &str, llm: Option<ExtReasoning>, web: Option<String>) -> ExtInsight {
    let (llm_type, suggested_class) = match &llm {
        Some(r) => (
            if r.type_desc.is_empty() {
                None
            } else {
                Some(r.type_desc.clone())
            },
            r.class.clone(),
        ),
        None => (None, None),
...
mod tests {
    ...
    fn distinct_extensions_lowercased_sorted_deduped() {
        ...
        assert_eq!(
            distinct_extensions(&s),
            vec!["fbx".to_string(), "parquet".to_string()]
        );
    }
    #[test]
    fn merge_prefers_web_type_keeps_llm_class() {
        let llm = Some(ExtReasoning {
            type_desc: "3D model".into(),
            class: Some("Model3D".into()),
        });
    ...
    fn merge_llm_only_and_web_only_and_none() {
        ...
        assert_eq!(
            merge_insight("dat", None, Some("desc".into())).source,
            "web"
        );
