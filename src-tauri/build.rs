...
const CLOUD_PLAN_IMPLEMENTATION: &str =
    "src/bin/disksage-cloud-plan-implementation.rs.inc";
...
fn generate_cloud_plan_implementation() {
    ...
    // `include!` cannot accept an inner `//!` document comment introduced by macro expansion.
    // Preserve the source file as documentation-oriented text, but compile an equivalent generated
    // copy whose first marker is an ordinary comment. Keep the plist path rooted at the crate
    // manifest because `include_bytes!` otherwise resolves relative to OUT_DIR after generation.
