...
fn brew_cleanup_plan_runs_off_the_tauri_main_thread() {
    ...
    assert!(
        attribute.contains("#[tauri::command(async)]"),
...
fn brew_cleanup_judgment_releases_engine_before_storing_authority() {
    ...
    assert!(infer < release, "engine lock may cover inference itself");
    assert!(
        release < store,
        "engine lock must be released before acquiring the judgment-authority lock"
    );
}
