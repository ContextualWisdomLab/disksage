...
fn write_fake_uv(path: &Path, cache: &Path, tools: &Path, prune_body: &str) {
    fs::write(
        path,
        format!(
            "#!/bin/sh\nset -eu\nif [ \"${{1:-}}\" = '--version' ]; then printf 'uv 0.test\\n'; exit 0; fi\nif [ \"${{1:-}} ${{2:-}}\" = 'cache dir' ]; then printf '%s\\n' '{}'; exit 0; fi\nif [ \"${{1:-}} ${{2:-}}\" = 'tool dir' ]; then printf '%s\\n' '{}'; exit 0; fi\nif [ \"${{1:-}} ${{2:-}}\" = 'cache prune' ]; then {}\nfi\nexit 64\n",
            cache.display(), tools.display(), prune_body
        ),
    )
    .unwrap();
...
fn fixture() -> (tempfile::TempDir, PathBuf, PathBuf, PathBuf) {
    ...
    (temp, cache, tools, home)
}

fn read_plan(binary: &str, uv: &Path, home: &Path) -> serde_json::Value {
...
fn successful_prune_with_result_record_failure_still_emits_terminal_receipt() {
    let (temp, cache, tools, home) = fixture();
    ...
    let plan = read_plan(binary, &uv, &home);
    let output = execute(
        binary,
        &uv,
        &home,
