...
//!
//! The writer is intentionally private: callers must reach it only through the bounded OAuth
//! lifecycle. Include the production module here so tests exercise the real atomic local
//! persistence boundary without widening the shipped API, opening a browser, contacting a provider,
//! or touching the credential store.
