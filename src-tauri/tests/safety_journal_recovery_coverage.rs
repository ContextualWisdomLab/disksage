//! Crash-recovery coverage for the destructive-operation audit journal.
//!
//! A process can be interrupted after writing a complete JSON record but before its trailing
//! newline reaches the journal. The next append must restore the record boundary before writing a
//! new entry so audit history remains independently parseable and ordered.
