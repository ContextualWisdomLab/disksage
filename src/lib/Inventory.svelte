<script lang="ts">
  ...
  async function loadUserRules() {
    try {
      ...
      userRulesCount = null;
      userRulesError = "사용자 규칙을 불러오지 못했습니다. 설정을 확인한 뒤 다시 시도하십시오.";
    }
  }
  ...
  async function load() {
    ...
    try {
      ...
      loadError = "인벤토리를 불러오지 못했습니다. 경로와 권한을 확인한 뒤 다시 시도하십시오.";
    } finally {
      busy = false;
    }
  }
  ...
  async function doDownload() {
    ...
    try {
      ...
      loadError = "모델을 다운로드하지 못했습니다. 연결 상태를 확인한 뒤 다시 시도하십시오.";
    } finally {
      modelBusy = false;
    }
  }
  ...
  async function summarizeUnknown() {
    ...
    try {
      ...
      summary = "요약을 만들지 못했습니다. 인벤토리를 다시 확인한 뒤 시도하십시오.";
    } finally {
      summaryLoaded = true;
      summaryBusy = false;
    }
...
<section>
  ...
  <div class="model-status">
    {#if model?.present}
      <span>분류 보조 기능 활성화 ✓</span>
    {:else}
      <button onclick={doDownload} disabled={modelBusy}>{modelBusy ? "분류 보조 기능 준비 중…" : "분류 보조 기능 준비"}</button>
    {/if}
    <span class="muted small">분류 결과는 참고용입니다. 결과를 확인한 뒤 정리 작업을 선택하십시오.</span>
  </div>
  ...
  {#if report}
      ...
      {#if report.unknown_count > 0}
        <li class="unknown">
          ...
          <div class="unknown-summary">
            ...
            {#if summaryLoaded}
              <span class="summary-text">{summary ?? "분류 보조 기능을 사용할 수 없습니다."}</span>
            {/if}
          </div>
    ...
    {#if issues !== null}
      <div class="coherence">
        {#if issues.length === 0}
          <span class="ok small">파일 분류 정합 ✓</span>
        {:else}
          <ul class="issues">
            {#each issues as i}
              <li class="warn">파일 분류 충돌이 발견되었습니다. 설정을 확인한 뒤 다시 집계하십시오.</li>
            {/each}
          </ul>
        {/if}
      </div>
