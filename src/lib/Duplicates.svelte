<script lang="ts">
  ...
  async function scan() {
    ...
    try {
      ...
      loadError = "중복 파일을 확인하지 못했습니다. 스캔 상태를 확인한 뒤 다시 시도하십시오.";
    } finally {
      busy = false;
    }
  }
  ...
  async function deleteSelected() {
    ...
    const okay = await confirm(
      `${paths.length}개 중복 파일을 휴지통으로 보냅니다 (예상 용량 ${fmtBytes(reclaimable)}).\n` +
        "각 그룹의 사본 1개는 보존됩니다. 휴지통을 비우기 전에는 복원할 수 있습니다.",
    ...
    try {
      ...
      loadError = "선택한 중복 파일을 휴지통으로 보내지 못했습니다. 목록을 확인한 뒤 다시 시도하십시오.";
    } finally {
      busy = false;
    }
  }
    ...
    {#if results.some((r) => !r.ok)}
      <ul class="errors">
        {#each results.filter((r) => !r.ok) as r (r.path)}
          <li title={r.path}>⚠ {r.path} — 휴지통으로 옮기지 못했습니다. 목록을 확인한 뒤 다시 시도하십시오.</li>
        {/each}
      </ul>
    {/if}
  {/if}
