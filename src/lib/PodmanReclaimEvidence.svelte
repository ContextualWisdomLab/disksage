<script lang="ts">
  ...
  import { podmanReclaimPlan, type PodmanReclaimPlan } from "./podmanApi";
  import {
    podmanActionLabel,
...
<section class="podman-evidence" aria-labelledby="podman-evidence-title">
  ...
  {#if plan}
    <div class="status" class:incomplete={!plan.evidence_complete} role="status">
...
<style>
  .podman-evidence { margin: 1rem 0; padding: 0.9rem; border: 1px solid #cfd8e3; border-radius: 8px; background: #f8fafc; }
