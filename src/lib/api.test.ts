...
describe("api wrappers", () => {
  ...
  it("forwards every command to Tauri with the expected payload shape", () => {
    ...
    const cases: Array<[() => unknown, string, unknown?]> = [
      ...
      [() => api.judgeOrphanCleanup(), "judge_orphan_cleanup"],
      [() => api.cleanOrphanCandidates("c".repeat(64), []), "clean_orphan_candidates", { planFingerprint: "c".repeat(64), requests: [] }],
      [
        () => api.executeBrewCleanup("a".repeat(64), "b".repeat(64), "DiskSage Homebrew cleanup 승인", "reviewed dry-run"),
        "execute_brew_cleanup",
        {
          planFingerprint: "a".repeat(64),
          judgmentId: "b".repeat(64),
          confirmationPhrase: "DiskSage Homebrew cleanup 승인",
