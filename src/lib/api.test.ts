...
describe("api wrappers", () => {
  ...
  it("forwards every command to Tauri with the expected payload shape", () => {
    ...
    const cases: Array<[() => unknown, string, unknown?]> = [
      ...
      [() => api.planProviderCacheReclaim(), "plan_provider_cache_reclaim"],
      [() => api.executeProviderCacheReclaim([], "a".repeat(64), "a".repeat(64), "exact", "reviewed exact cache", "permanent_purge"), "execute_provider_cache_reclaim", { requests: [], approvedPlanFingerprint: "a".repeat(64), confirmPlanFingerprint: "a".repeat(64), confirmationPhrase: "exact", rationale: "reviewed exact cache", mode: "permanent_purge" }],
