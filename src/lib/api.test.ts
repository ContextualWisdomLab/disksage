...
describe("api wrappers", () => {
  ...
  it("forwards every command to Tauri with the expected payload shape", () => {
    ...
    const cases: Array<[() => unknown, string, unknown?]> = [
      ...
      [() => api.planExactPhotoDuplicateQuarantine({} as api.PhotoDuplicateAudit, []), "plan_exact_photo_duplicate_quarantine", { audit: {}, selections: [] }],
      [() => api.executeExactPhotoDuplicateQuarantine({} as api.PhotoDuplicateAudit, {} as api.PhotoQuarantinePlan, "typed", "reviewed", 9), "execute_exact_photo_duplicate_quarantine", { audit: {}, plan: {}, approvalPhrase: "typed", rationale: "reviewed", executedAtMs: 9 }],
