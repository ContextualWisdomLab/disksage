...
describe("api wrappers", () => {
  ...
  it("forwards every command to Tauri with the expected payload shape", () => {
    ...
    const cases: Array<[() => unknown, string, unknown?]> = [
      ...
      [() => api.executeProviderCacheReclaim([], "a".repeat(64), "a".repeat(64), "exact", "reviewed exact cache"), "execute_provider_cache_reclaim", { requests: [], approvedPlanFingerprint: "a".repeat(64), confirmPlanFingerprint: "a".repeat(64), confirmationPhrase: "exact", rationale: "reviewed exact cache", mode: "trash" }],
