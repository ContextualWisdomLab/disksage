const INSPECT_FAILURE_MESSAGE = "런타임 확인 실패 — 잠시 후 다시 시도해 주세요.";
const PRUNE_FAILURE_MESSAGE = "정리 실행 실패 — 데이터는 그대로입니다.";
...
export function containerOrphanPruneErrorMessage(error: unknown): string {
  ...
  if (detail.startsWith("orphan-prune-confirmation-mismatch")) {
    return "승인 문구가 최신 증거와 일치하지 않습니다. 새로 확인한 뒤 문구를 다시 입력해 주세요.";
  }
  ...
  if (detail.startsWith("orphan-prune-evidence-incomplete")) {
    return "증거가 불완전해 실행이 중단되었습니다. 런타임 상태를 확인한 뒤 다시 시도해 주세요.";
  }
