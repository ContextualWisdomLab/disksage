<script lang="ts">
  ...
  async function load() {
    ...
    try {
      ...
      loadError = "정리 후보를 불러오지 못했습니다. 상태를 확인한 뒤 다시 시도하십시오.";
    }
  }
    ...
    try {
      ...
      const okay = await confirm(
        `${candidate.label}의 직계 캐시 ${targets.length}개(${fmtBytes(targetBytes)})를 휴지통으로 보냅니다.\n\n` +
          "캐시 루트는 보존하며, 각 항목은 크기와 변경 여부를 다시 확인합니다. 사용 중이거나 확인이 불완전한 항목은 건너뜁니다. 휴지통에서 복원할 수 있습니다.",
      ...
      if (error.includes("cache-cleanup-targets-stale")) {
        ...
        loadError = "캐시를 휴지통으로 이동하지 못했습니다. 상태를 확인한 뒤 다시 시도하십시오.";
      }
    } finally {
      busy = false;
    }
  ...
  async function cleanRegenerableCaches() {
    ...
    try {
      ...
      loadError = "재생성 가능한 캐시를 정리하지 못했습니다. 상태를 확인한 뒤 다시 시도하십시오.";
    } finally {
      busy = false;
    }
  }
  ...
  async function executeClean() {
    ...
    const okay = await confirm(
      ...
      "\n\n휴지통에서 언제든 복원할 수 있습니다. 실제 저장 공간을 회수하려면 휴지통을 비운 뒤 저장 공간을 새로고침하십시오.",
      { title: "DiskSage", kind: "warning" },
    );
    if (!okay) return;
    ...
    try {
      ...
      loadError = "개발 아티팩트를 휴지통으로 이동하지 못했습니다. 상태를 확인한 뒤 다시 시도하십시오.";
    } finally {
      busy = false;
    }
  }
...
<section>
  ...
  <p class="notice" role="status">
    npm·pnpm·Adobe·Edge·uv·Trivy 캐시만 대상으로 하며, 사용 중이거나 확인이 바뀐 항목은 자동으로 건너뜁니다. 후보를 확인한 뒤 정리를 실행하십시오.
  </p>
  ...
  <h3>Podman 저장 공간</h3>
  <p class="notice">
    Podman 저장 공간 상태만 확인합니다. 실행 전에 정리 범위와 영향을 검토하십시오.
    실제 저장 공간 변화는 정리 후 새로고침하여 확인하십시오.
  </p>
  ...
  {#if podmanPlan}
    <div class="podman-evidence" aria-live="polite">
      <p>
        ...
        사용 가능 공간 {podmanPlan.guest_filesystem ? fmtBytes(podmanPlan.guest_filesystem.available_bytes) : "확인 불가"} ·
        확인된 회수 가능 공간 {podmanPlan.assessment.podman_reported_reclaimable_bytes === null
          ? "미확인"
          : fmtBytes(podmanPlan.assessment.podman_reported_reclaimable_bytes)}
      </p>
      {#if podmanPlan.unused_images}
        <p>미사용 이미지 {podmanPlan.unused_images.unused_records}개 · 기록 합계 {fmtBytes(podmanPlan.unused_images.candidate_record_size_sum)}</p>
      {/if}
      {#if podmanPlan.dangling_prune_approval_phrase}
        <div class="podman-prune">
          <p>확인된 미사용 이미지만 실행 대상으로 선택되었습니다.</p>
          <label>정확한 승인 문구
          ...
          <button onclick={prunePodmanDanglingImages} disabled={!podmanPruneReady()}>
            {podmanPruneBusy ? "재검증 후 미사용 이미지 정리 중…" : "미사용 이미지 정리"}
          </button>
      ...
      {#if podmanPlan.assessment.recommended_actions.length > 0}
        <ul class="errors">
          {#each podmanPlan.assessment.recommended_actions as action (action.kind)}
            <li>{podmanActionLabel(action)} — 범위를 확인한 뒤 실행하십시오.</li>
          {/each}
        </ul>
      {/if}
    </div>
