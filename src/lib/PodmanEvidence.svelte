...
<section class="podman-evidence" aria-labelledby="podman-evidence-title">
  ...
  {#if evidence && view}
    ...
    <dl class="metrics">
      <div>
        <dt>설정된 머신 디스크</dt>
        <dd>{optionalBytes(evidence.capacity.configured_disk_bytes)}</dd>
      </div>
      <div>
        <dt>Raw 이미지 논리 크기</dt>
        <dd>{optionalBytes(evidence.capacity.raw_logical_bytes)}</dd>
      </div>
      <div>
        <dt>호스트 할당 블록</dt>
        <dd>{optionalBytes(evidence.capacity.host_allocated_bytes)}</dd>
      </div>
      <div>
        <dt>게스트 파일시스템 전체</dt>
        <dd>{optionalBytes(evidence.capacity.guest_total_bytes)}</dd>
      </div>
      <div>
        <dt>게스트 파일시스템 사용</dt>
        <dd>{optionalBytes(evidence.capacity.guest_used_bytes)}</dd>
      </div>
      <div>
        <dt>게스트 파일시스템 여유</dt>
        <dd>{optionalBytes(evidence.capacity.guest_available_bytes)}</dd>
      </div>
      <div>
        <dt>Podman graph root 할당</dt>
        <dd>{optionalBytes(evidence.capacity.graph_root_allocated_bytes)}</dd>
      </div>
      <div>
        <dt>Podman graph root 사용</dt>
        <dd>{optionalBytes(evidence.capacity.graph_root_used_bytes)}</dd>
      </div>
      <div>
        <dt>Raw 할당−게스트 사용 차이</dt>
        <dd>{optionalBytes(evidence.raw_allocated_minus_guest_used_bytes)}</dd>
      </div>
      <div>
        <dt>Podman 논리 후보 합계</dt>
        <dd>{optionalBytes(evidence.podman_reported_reclaimable_bytes)}</dd>
      </div>
    </dl>
    ...
    <div class="review-grid">
      <article>
        <h5>이미지</h5>
        <p>{view.image_review_label}</p>
        <dl>
          <div><dt>논리 후보</dt><dd>{optionalBytes(evidence.candidates.image_candidate_bytes)}</dd></div>
          <div><dt>참조 0 레코드</dt><dd>{optionalCount(evidence.candidates.unused_image_records)}</dd></div>
        </dl>
      </article>
      <article>
        <h5>중지 컨테이너</h5>
        <p>{view.container_review_label}</p>
        <dl>
          <div><dt>논리 후보</dt><dd>{optionalBytes(evidence.candidates.stopped_container_candidate_bytes)}</dd></div>
          <div><dt>중지 레코드</dt><dd>{optionalCount(evidence.candidates.stopped_container_records)}</dd></div>
        </dl>
      </article>
      <article>
        <h5>로컬 볼륨</h5>
        <p>{view.volume_review_label}</p>
        <dl>
          <div><dt>논리 후보</dt><dd>{optionalBytes(evidence.candidates.volume_candidate_bytes)}</dd></div>
        </dl>
      </article>
    </div>
    ...
    <p>
      이미지 후보 집합 SHA-256:
      {#if evidence.candidates.image_candidate_set_sha256}
        <code>{evidence.candidates.image_candidate_set_sha256}</code>
      {:else}
        <span>관측되지 않음</span>
      {/if}
    </p>
