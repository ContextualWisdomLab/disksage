...
<div class="local-eviction-panel">
  ...
  <p class="muted">
    이미 iCloud에 있는 파일의 로컬 사본만 확인합니다. 파일 내용과 클라우드 파일은 변경하지 않습니다.
  </p>
      ...
      {#if eviction}
        {#if eviction.result.verification_complete}
          <p class="safe">
            ...
            iCloud 파일과 클라우드 연결은 유지됩니다.
          </p>
        {:else}
          <div class="warning" role="alert">
            <p>로컬 공간 회수 요청은 처리됐지만 저장 공간 감소를 확인하지 못했습니다. 저장 공간을 직접 확인하세요.</p>
            <ul>
      ...
      {:else if plan.eligible_after_human_approval}
        <div class="approval-controls">
          ...
          <button onclick={evictLocalCopy} disabled={!executionReady()}>
            {executing ? "상태 재확인 후 회수 중…" : "다시 확인하고 로컬 사본만 회수"}
          </button>
        </div>
      {:else}
        <div class="warning" role="status">
          <p>현재 로컬 사본을 회수할 수 없습니다. 다음 항목을 확인하세요.</p>
          <ul>
