<script lang="ts">
  ...
  function candidateLabel(candidate: api.OrphanCandidate): string {
    return candidate.bundle_id ? "macOS 앱 캐시" : "확인된 캐시";
  }
  ...
  async function inspect() {
    ...
    try {
      ...
      plan = null;
      error = "사용하지 않는 캐시 후보를 조사하지 못했습니다. macOS 파일 접근 권한을 확인한 뒤 다시 시도하세요.";
    } finally {
      busy = false;
    }
  }
  ...
  async function clean() {
    ...
    const okay = await confirm(
      `${selectedCandidates.length}개의 확인된 미사용 캐시(${fmtBytes(selectedBytes)})만 휴지통으로 보냅니다. 앱이 사용하는 데이터와 확인되지 않은 항목은 포함되지 않습니다.`,
      { title: "DiskSage 캐시 정리", kind: "warning" },
    );
    if (!okay) {
      busy = false;
      return;
    ...
    try {
      ...
      if (outcome.refresh_failed) {
        error = "휴지통 이동 요청은 처리됐지만 새 후보 목록을 불러오지 못했습니다. 이동 결과를 확인한 뒤 다시 조사하세요.";
      }
    } catch {
      error = "선택한 캐시를 휴지통으로 이동하지 못했습니다. 후보를 다시 조사한 뒤 재시도하세요.";
    } finally {
      busy = false;
    }
  }
...
<section>
  ...
  <p class="notice">
    설치된 앱 정보와 캐시 파일 정보를 비교해 사용하지 않는 후보를 찾습니다. 파일 내용은 읽지 않으며 개인 파일 경로를 결과에 포함하지 않습니다.
    앱이 사용하는 데이터와 확인되지 않은 후보는 자동 정리하지 않습니다. 목록을 확인한 뒤 이동할 항목을 선택하세요.
  </p>
  <button onclick={inspect} disabled={busy}>{busy ? "캐시 후보 조사 중…" : "캐시 후보 조사"}</button>
  {#if error}<p class="error" role="alert">작업을 처리하지 못했습니다. 다시 시도하세요. {error}</p>{/if}
  {#if plan}
      ...
      {#each plan.candidates as candidate (candidate.candidate_id)}
        <li>
          <label class:disabled={!plan.scan_complete || !candidate.auto_trash_eligible}>
            ...
            />
            {candidateLabel(candidate)} · {fmtBytes(candidate.bytes)}
          </label>
          <span class="muted">
            {candidate.auto_trash_eligible ? "확인된 미사용 캐시" : "추가 확인 필요 — 다시 검토하세요."}
          </span>
        </li>
      {/each}
    </ul>
    {#if selectedCandidates.length > 0}
      <p class="muted">표시된 승인 문구를 그대로 입력한 뒤 실행하세요: <code>{plan.exact_approval_phrase}</code></p>
