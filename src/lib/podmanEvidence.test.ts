...
function completePlan(): PodmanReclaimPlan {
  return {
    ...
    elapsed_ms: 250,
    machine: { name: "private-machine-name", state: "running", configured_disk_bytes: 1000 },
    raw_image: { path: "/Users/private/.local/share/podman/raw-image", logical_bytes: 900, allocated_bytes: 700 },
    ...
    assessment: {
      ...
      recommended_actions: [
        { kind: "review_unused_images", requires_human_approval: true, rationale: "Review image candidates separately." },
      ],
    },
    issues: ["podman-info-failed:/Users/private/socket", "guest-df-empty", "guest-df-empty"],
  };
}
...
describe("Podman evidence presentation", () => {
  ...
  it("reduces detailed failures to bounded path-free issue codes", () => {
    expect(safePodmanIssueCode("podman-info-failed:/Users/private/socket")).toBe("podman-info-failed");
    ...
    expect(podmanIssueCodes(completePlan())).toEqual(["guest-df-empty", "podman-info-failed"]);
  });
  ...
  it("keeps configured, observed, logical-candidate, and physical-proof metrics distinct", () => {
    expect(podmanEvidenceMetrics(completePlan())).toEqual([
  ...
  it("keeps image, stopped-container, and volume review approvals separate", () => {
    ...
    expect(podmanCandidateCategories(emptyPlan()).map((category) => category.bytes)).toEqual([null, null, null]);
  });
});
