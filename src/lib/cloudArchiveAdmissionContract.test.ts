...
describe("CloudArchive iCloud admission contract", () => {
  it("clears stale health evidence when refresh fails", () => {
    ...
    expect(source).toContain("iCloud 동기화가");
    expect(source).toContain("icloud-item-error-octagon-not-signed-in");
    expect(source).toContain("동기화 상태 확인:");
    expect(source).toContain("iCloud 상태를 확인할 수 없음");
    ...
    expect(source).toContain("Finder가 “복사 준비 중”에서 멈췄습니다.");
    expect(source).not.toContain("Finder가 “복사 준비 중”에서 멈춘 원인은");
    expect(source).toContain("Finder에 남은 복사 대기를 취소");
    expect(source).toContain("iCloud 상태 확인이 늦어짐");
    expect(source).toContain("파일과 클라우드 연결 정보");
    expect(source).toContain("파일 확인 → 클라우드 복사 → 원본 정리");
    expect(source).toContain("candidate.metadata_fingerprint");
    expect(source).toContain("마지막 상태 확인:");
    ...
    expect(source).toContain("iCloud 파일이 잠겨 있음");
    expect(source).toContain("iCloud 파일이 잠겨 Finder 복사가 늦어지고 있습니다.");
    expect(source).toContain("iCloud 파일 전송 오류가 15분 이상 지속되었습니다.");
    ...
    expect(source).toContain("확인이 필요한 이유:");
    ...
    expect(source).toContain("접근 불가 클라우드 루트는 선택에서 제외했습니다.");
    expect(source).toContain("클라우드 앱 재시작 후 상태 확인");
