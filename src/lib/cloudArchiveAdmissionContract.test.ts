...
describe("CloudArchive iCloud admission contract", () => {
  it("clears stale health evidence when refresh fails", () => {
    ...
    expect(source).toContain("동기화 확인이 15분 이상 완료되지 않았습니다.");
    expect(source).toContain("클라우드 동기화 상태를 확인하지 못했습니다.");
    expect(source).toContain("로컬 파일을 그대로 두고 공급자 동기화가 끝난 뒤 다시 검사하십시오.");
