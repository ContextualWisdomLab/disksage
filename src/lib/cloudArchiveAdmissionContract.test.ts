...
describe("CloudArchive iCloud admission contract", () => {
  it("clears stale health evidence when refresh fails", () => {
    ...
    expect(source).toContain("이 시스템 데이터를 삭제하지 않습니다");
    expect(source).toContain("icloud-item-error-octagon-not-signed-in");
    expect(source).toContain("동기화 상태를 확인한 뒤");
    ...
    expect(source).toContain("Finder의 파일 복사 준비가 멈춰 있습니다.");
    expect(source).not.toContain("Finder가 “복사 준비 중”에서 멈춘 원인은");
    expect(source).toContain("대기 중인 복사를 취소");
    expect(source).toContain("iCloud 상태 확인이 오래 걸립니다");
    expect(source).toContain("파일 관계 확인");
    expect(source).toContain("파일 확인 → 업로드 확인 → 원본 이동 확인");
    ...
    expect(source).toContain("같은 iCloud 문제가 15분 이상 지속되었습니다.");
    ...
    expect(source).toContain("iCloud 파일 처리가 잠겨 있음");
    expect(source).toContain("iCloud 파일 처리가 잠겨 복사 준비가 지연되고 있습니다.");
    expect(source).toContain("iCloud 파일 처리가 15분 이상 멈춰 있습니다.");
    ...
    expect(source).toContain("같은 클라우드 문제가 15분 이상 지속되었습니다.");
    expect(source).toContain("클라우드 전체 상태를 확인하지 못했습니다.");
    ...
    expect(source).toContain("연결 상태를 확인하고 목록을 새로 고치세요.");
    ...
    for (const internalTerm of [
      ...
      expect(customerMarkup).not.toContain(internalTerm);
    }
  });
