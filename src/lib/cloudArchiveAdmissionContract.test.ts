...
describe("CloudArchive iCloud admission contract", () => {
  it("clears stale health evidence when refresh fails", () => {
    ...
    expect(source).toContain("시스템 데이터를 삭제하지 않습니다");
    expect(source).toContain("icloud-item-error-octagon-not-signed-in");
    expect(source).toContain("동기화 확인:");
    ...
    expect(source).toContain("Finder의 복사 준비가 멈춰 있습니다.");
    expect(source).not.toContain("Finder가 “복사 준비 중”에서 멈춘 원인은");
    expect(source).toContain("Finder에 남은 복사 대기를 취소하고");
    expect(source).toContain("클라우드 상태 확인이 오래 걸립니다");
    expect(source).toContain("원본과 저장 위치");
    expect(source).toContain("파일을 복사하고 업로드를 확인한 뒤 원본 이동을 별도로 승인합니다.");
    ...
    expect(source).toContain("iCloud 처리가 15분 이상 멈춰 있습니다.");
    ...
    expect(source).toContain("클라우드 파일 처리가 잠겨 있습니다.");
    expect(source).toContain("Finder의 대기 작업을 취소하고");
    expect(source).toContain("클라우드 파일 처리가 15분 이상 멈춰 있습니다.");
    expect(source).toContain("icloud-file-provider-stalled");
    expect(source).not.toContain("Finder의 복사 준비가 진행되지 않습니다.");
    expect(source).toContain("같은 상태가 지속된 시간");
    expect(source).toContain("클라우드 처리가 15분 이상 멈춰 있습니다.");
    expect(source).toContain("클라우드 전체 동기화 상태를 확인하지 못했습니다.");
    ...
    expect(source).toContain("클라우드 앱 상태 확인과 앱 재시작만 가능하며");
    ...
    const customerMarkup = source.slice(source.indexOf("</script>"), source.indexOf("<style>"));
    expect(customerMarkup).not.toContain("Goal:");
    expect(customerMarkup).not.toContain("동적 ADR");
    expect(customerMarkup).not.toContain("Lineage 연결관계");
    expect(customerMarkup).not.toContain("File Provider");
    expect(customerMarkup).not.toContain("OAuth");
    expect(customerMarkup).not.toContain("계획 지문");
    expect(customerMarkup).not.toContain("스키마 표본");
    expect(customerMarkup).not.toContain("API attestation");
  });
