...
/** Read one canonical Rust source file for the module-boundary ownership contract. */
function readRustSource(path: string): string {
  return readFileSync(resolve(repositoryRoot, path), "utf8");
}

describe("LLM parse export ownership", () => {
  it("keeps the legacy verdict parser private while retaining the consumed full verdict parser", () => {
    ...
    // The parser itself stays available to its owning module and tests; this slice removes only
    // an unconsumed cross-module re-export, not verdict semantics.
    expect(parseSource).toContain("pub fn parse_verdict(");
  });
});
