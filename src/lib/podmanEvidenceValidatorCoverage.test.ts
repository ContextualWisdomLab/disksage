...
describe("Podman desktop evidence validator failure coverage", () => {
  it("rejects primitive, null, array, and null nested records", () => {
    expect(() => parsePodmanDesktopEvidence("not-a-record")).toThrow("invalid-evidence");
    expect(() => parsePodmanDesktopEvidence(null)).toThrow("invalid-evidence");
    expect(() => parsePodmanDesktopEvidence([])).toThrow("invalid-evidence");

    const nestedNull = cloneEvidence();
    nestedNull.capacity = null;
    expect(() => parsePodmanDesktopEvidence(nestedNull)).toThrow("invalid-capacity");
  });
