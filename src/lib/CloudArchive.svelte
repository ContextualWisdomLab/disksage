...
<section>
  ...
  {#if report}
    ...
    <p class="warning">
      내장 고신뢰 생산일을 우선합니다. 파일명 날짜는 생산일로 승격하지 않고 충돌 검토 힌트로만 보존합니다. 파일시스템 생성·수정 시각 fallback은 현재 메타데이터와 목적지에 결박된 명시적 승인이 있어야만 복사할 수 있습니다. 원본 삭제 기능은 제공하지 않으며, 업로드 증거가 확인되어도 허가 정보만 표시합니다.
    </p>
    {#if copied}
      <div class="receipt">
