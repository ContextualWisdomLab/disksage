<script lang="ts">
  ...
  function capacityUnavailableLabel(reason: string | null): string {
    const labels: Record<string, string> = {
      ...
      "icloud-native-quota-unsupported-platform": "iCloud 저장 공간 확인은 macOS에서만 지원됩니다.",
  ...
  function productionSourceLabel(source: string): string {
    ...
    if (source.startsWith("filename:")) return "파일명";
    if (source.startsWith("filesystem-created:")) return "파일 생성 시각";
    if (source.startsWith("filesystem-modified:")) return "파일 수정 시각";
    return "확인되지 않은 정보";
  }
  ...
  function sourceContextLabel(context: string): string {
    return context.includes("download") ? "다운로드 파일" : context.includes("archive") ? "보관 파일" : "현재 파일";
  }
</script>
...
<section>
  ...
  <p class="muted">
    iCloud Drive·OneDrive·Google Drive의 폴더를 찾고, 파일 정보를 우선하여 생산 시점과 원래 폴더 위치를 보존하는 이동 계획만 만듭니다.
  </p>

  {#if roots.length === 0}
    <p class="warning">클라우드 폴더를 찾지 못했습니다. 연결 상태와 파일 접근 권한을 확인한 뒤 목록을 다시 불러오세요.</p>
  {:else}
    <div class="controls">
      <label>
        대상
    ...
    {#if selectedRootDetails() && !selectedRootDetails()?.readable}
      <p class="warning">
        이 클라우드 폴더를 현재 읽을 수 없습니다. 연결 상태를 확인하고 목록을 새로 고치세요.
        확인이 끝날 때까지 복사와 원본 정리는 잠시 멈춥니다.
      </p>
    {/if}
    {#if reconciliation}
      <div class="receipt-reconciliation" aria-live="polite">
        ...
        <span class="context">
          {reconciliation.receipts_seen}개 확인 · {reconciliation.attested_count}개 업로드 상태 확인 ·
          {reconciliation.pending_count}개 업로드 대기 · {reconciliation.error_count}개 확인 실패
          {#if reconciliation.incomplete_reconciliation} · {reconciliation.unprocessed_count}개 추가 확인 필요{/if}
        </span>
        {#if reconciliation.entries.length === 0}
          <p class="muted">이전에 확인된 작업이 없습니다. 새 계획을 실행해 보세요.</p>
        {:else}
          ...
          {/each}
        {/if}
        <p class="muted">이 확인은 상태만 갱신하며 클라우드에 쓰거나 원본을 삭제하지 않습니다.</p>
      </div>
    {/if}
      ...
      <div class="receipt-reconciliation" aria-live="polite">
        ...
        <span class="context">
          ...
          {#if icloudHealth.file_provider_activity}
            · 처리가 멈춘 항목 {icloudHealth.file_provider_activity.no_progress_fetch_count + icloudHealth.file_provider_activity.no_progress_create_count}개 ·
            파일 준비 실패 {icloudHealth.file_provider_activity.materialization_failure_count}개 / 준비되지 않은 파일 {icloudHealth.file_provider_activity.staged_item_missing_count}개 ·
            업로드 진행 {icloudHealth.file_provider_activity.active_upload_count}개 / 다운로드 진행 {icloudHealth.file_provider_activity.active_download_count}개
        ...
        {#if hasIcloudHealthEvidencePersistenceFailure(icloudHealth.notices)}
          <p class="warning">
            iCloud 동기화 확인 정보를 저장하지 못했습니다. 이번 상태는 표시하되 장기 비교에는 사용하지 않으며,
            복사와 원본 정리는 확인 정보가 다시 저장될 때까지 보류합니다.
          </p>
        {/if}
        {#if icloudHealth.new_copy_admission_blockers.length > 0}
          <p class="warning">
            지금 진행할 수 없습니다. 다음 확인 항목을 해결하세요:
          ...
          {#if icloudHealth.file_provider_activity && (icloudHealth.file_provider_activity.no_progress_fetch_count > 0 || icloudHealth.file_provider_activity.no_progress_create_count > 0)}
            <p class="warning">
              Finder의 파일 복사 준비가 멈춰 있습니다. 대기 중인 복사를 취소하고,
              iCloud 상태를 다시 확인한 뒤 새 계획을 실행하세요.
            </p>
          {/if}
          {#if icloudHealth.file_provider_activity && (icloudHealth.file_provider_activity.materialization_failure_count > 0 || icloudHealth.file_provider_activity.staged_item_missing_count > 0)}
            <p class="warning">
              iCloud가 파일 준비를 끝내지 못했습니다. 현재 복사는 완료로 간주하지 않으며,
              상태가 정상으로 확인될 때까지 새 복사와 원본 정리를 시작하지 마세요.
            </p>
          {/if}
          {#if icloudHealth.new_copy_admission_blockers.includes("icloud-file-provider-item-locked")}
            <p class="warning">
              iCloud 파일 처리가 잠겨 복사 준비가 지연되고 있습니다. 대기 작업을 취소하고,
              상태가 정상화된 뒤 새 복사를 다시 시작하세요.
            </p>
          {/if}
          {#if icloudHealth.new_copy_admission_blockers.includes("icloud-file-provider-stalled")}
            <p class="warning">
              iCloud 파일 처리가 15분 이상 멈춰 있습니다. 대기 중인 복사를 취소하고,
              상태가 정상화된 뒤 새 복사를 다시 시작하세요.
            </p>
          {/if}
          {#if icloudHealth.file_provider_activity?.timed_out}
            <p class="warning">
              iCloud 상태 확인이 오래 걸립니다. 대기 중인 복사를 취소하고,
              상태를 다시 확인한 뒤 새 복사를 시작하세요.
            </p>
          {/if}
          {#if icloudHealth.file_provider_activity && (icloudHealth.file_provider_activity.active_upload_count > 0 || icloudHealth.file_provider_activity.active_download_count > 0)}
            <p class="warning">
              iCloud에서 다른 업로드·다운로드가 진행 중입니다. 전송이 끝나고 상태가 정상으로 확인될 때까지
              Finder 복사와 원본 정리를 진행하지 마세요.
            </p>
          {/if}
          {#if icloudHealthBlockedSinceMs > 0 && icloudHealth.observed_at_ms - icloudHealthBlockedSinceMs >= PROVIDER_STALL_WARNING_MS}
            <p class="warning">
              같은 iCloud 문제가 15분 이상 지속되었습니다. 대기 중인 복사를 취소하고,
              iCloud 상태가 정상으로 확인될 때까지 새 복사와 원본 정리를 시작하지 마세요.
            </p>
          {/if}
        {:else}
          <p class="capacity-ok">iCloud 업로드 대기 항목이 없습니다. 파일별 상태를 확인한 뒤 진행하세요.</p>
        {/if}
        {#if typeof icloudHealth.managed_database_allocated_bytes === "number"}
          <p class="warning">
            ...
            DiskSage는 이 시스템 데이터를 삭제하지 않습니다. 다른 정리 후보를 확인하세요.
          </p>
        {/if}
        {#if icloudHealth.notices.some((notice) => notice.startsWith("icloud-item-error-"))}
          <p class="warning">
            동기화 상태를 확인한 뒤 파일별 상태를 다시 확인하세요:
        ...
        {/if}
        <p class="muted">읽기 전용 확인 정보입니다. 남은 공간과 파일 업로드 완료 여부를 확인한 뒤 다음 단계를 진행하세요.</p>
      </div>
    {/if}
    ...
    {#if providerGlobalSync}
      <div class="receipt-reconciliation" aria-live="polite">
        ...
        {#if providerGlobalSync.blockers.length > 0}
          <p class="warning">
            지금 진행할 수 없습니다. 다음 확인 항목을 해결하세요: {providerGlobalSync.blockers.map(providerGlobalSyncBlockerLabel).join(", ")}
          </p>
          {#if providerGlobalSyncBlockedSinceMs > 0 && providerGlobalSyncObservedAtMs - providerGlobalSyncBlockedSinceMs >= PROVIDER_STALL_WARNING_MS}
            <p class="warning">
              같은 클라우드 문제가 15분 이상 지속되었습니다. 대기 중인 복사를 취소하고,
              클라우드 앱을 다시 시작한 뒤 상태가 정상으로 확인될 때까지 새 복사와 원본 정리를 시작하지 마세요.
            </p>
          {/if}
          {#if selectedRootDetails()?.provider !== "icloud"}
            <button onclick={recoverProviderClient} disabled={recoveringProvider || checkingProviderGlobalSync}>
              {recoveringProvider ? "클라우드 앱 다시 시작 중…" : "클라우드 앱 재기동 후 상태 재확인"}
            </button>
            {#if canCancelFinderCopyForProviderGlobalSync(providerGlobalSync)}
              ...
              {#if finderCopyCancelStatus}<p class="muted">복사 취소 요청을 확인했습니다.</p>{/if}
            {/if}
          {/if}
        {:else}
          <p class="capacity-ok">클라우드 전체 동기화 대기열이 비어 있습니다. 파일별 상태를 확인한 뒤 진행하세요.</p>
        {/if}
        {#if providerRecovery}
          <p class:warning={providerRecovery.blockers.length > 0} class="muted">
            클라우드 앱 다시 시작 요청 완료 · 상태 재확인
            {providerRecovery.post_runtime_observed === true ? "확인됨" : "아직 확인되지 않음"}
            {#if providerRecovery.blockers.length > 0} · 추가 확인 필요{/if}
          </p>
        {/if}
        <p class="muted">읽기 전용 확인 정보입니다. 파일 업로드 완료 여부를 확인한 뒤 다음 단계를 진행하세요.</p>
      </div>
    {/if}
    {#if providerGlobalSyncError}
      <p class="error" role="alert">클라우드 전체 상태를 확인하지 못했습니다.</p>
      <p class="warning">
        클라우드 전체 상태를 확인하지 못했습니다. 대기 중인 복사를 취소하고,
        클라우드 앱이 정상으로 확인될 때까지 새 복사와 원본 정리를 시작하지 마세요.
      </p>
    {/if}
    {#if roots.some((root) => !root.readable)}
      <p class="warning">
        접근할 수 없는 클라우드 위치는 선택에서 제외했습니다. macOS 파일 접근 권한을 허용한 뒤 목록을 다시 불러오세요.
      </p>
    {/if}
    ...
    {#if selectedRootDetails()?.provider === "icloud"}
      <div class="oauth-panel">
        ...
        <button onclick={verifyProviderCapacity} disabled={checkingCapacity}>
          {checkingCapacity ? "iCloud 계정 확인 중…" : "iCloud 남은 공간 확인"}
        </button>
        {#if capacityForSelectedRoot()?.evidence_kind === "provider-native-status"}
          <p class="capacity-ok">
            Apple 계정 상태 확인 완료
            · 남은 공간 {fmtBytes(capacityForSelectedRoot()?.remaining_bytes ?? 0)}
          </p>
        ...
        {:else}
          <p class="muted">추가 계정 연결 없이 macOS의 읽기 전용 iCloud 상태를 확인합니다.</p>
        {/if}
        {#key selectedRoot}
    ...
    {:else if selectedRootDetails()}
      <div class="oauth-panel">
        {#if connectionForSelectedRoot()}
          ...
          <span class="context">{providerApiWriteConnected() ? "업로드를 사용할 수 있습니다." : "파일을 확인할 수 있는 연결입니다."}</span>
          <button
            onclick={verifyProviderCapacity}
            disabled={checkingCapacity || disconnecting || connecting}
          >
            {checkingCapacity ? "연결 상태와 클라우드 공간 확인 중…" : "연결 상태와 클라우드 공간 다시 확인"}
          </button>
          ...
          {:else}
            <p class="muted">
              연결 정보만 확인했습니다. 클라우드 앱을 다시 시작한 뒤 연결과 저장 공간을 확인하세요.
            </p>
          {/if}
        {:else}
          <label>
          ...
          <p class="muted">
            연결 정보는 이 기기에 안전하게 보관하며, 필요하면 언제든 연결을 해제할 수 있습니다.
          </p>
          {#if selectedRootDetails()?.provider === "onedrive"}
            <p class="muted">OneDrive에서 파일을 보관하려면 Microsoft 계정의 파일 권한을 허용하세요.</p>
          {/if}
          {#if selectedRootDetails()?.provider === "google-drive"}
            <p class="warning">파일을 보관하려면 Google Drive 파일 권한이 필요합니다. 확인만 하려면 권한을 허용하지 않아도 됩니다.</p>
          {/if}
        {/if}
      </div>
    {/if}
  ...
  {#if report}
    ...
    {#if hasProviderAdmissionBlocker(report.notices)}
      <p class="warning">
        클라우드 전체 동기화 확인이 끝나지 않았거나 전송 중입니다. 새 복사 버튼은 잠시 비활성화됩니다.
        상태가 정상으로 확인된 뒤 다시 계획하세요. 이미 있는 클라우드 파일 확인은 별도로 진행할 수 있습니다.
      </p>
    {/if}
    ...
    {#if report.local_volume}
      <p class:warning={report.local_volume.pressure !== "normal"}>
        원본 저장 공간 상태: {localPressureLabel(report.local_volume.pressure)} · 사용 가능
      ...
      {#if hasLocalEvidencePersistenceFailure(report.notices)}
        <p class="warning">
          저장 공간 확인 결과를 저장하지 못했습니다. 현재 결과를 확인한 뒤 필요하면 다시 계획하십시오.
        </p>
      {/if}
      {#if report.candidates.some(nativeCopyHeadroomBlocked)}
        <p class="warning">
        클라우드 폴더 복사는 후보 크기와 {fmtBytes(api.LOCAL_COPY_RESERVE_BYTES)} 여유 공간을 함께 확보해야 합니다.
        현재 여유 공간이 부족한 후보는 버튼을 비활성화합니다. 연결된 클라우드 업로드는 별도 경로로 진행할 수 있습니다.
        </p>
      {/if}
    {/if}
    {#if hasRuntimeEvidencePersistenceFailure(report.notices)}
      <p class="warning">
        클라우드 연결 상태를 저장하지 못했습니다. 현재 결과를 확인한 뒤 상태가 다시 저장되면 복사·동기화를 진행하세요.
      </p>
    {/if}
    {#if hasIcloudHealthEvidencePersistenceFailure(report.notices)}
      <p class="warning">
        iCloud 동기화 확인 결과를 저장하지 못했습니다. 현재 결과를 확인한 뒤 상태가 정상으로 관찰되면 다시 계획하십시오.
        상태가 정상으로 관찰된 뒤 다시 계획하십시오.
      </p>
    {/if}
    {#if report.pre_copy_evidence && !report.pre_copy_evidence.complete}
      <p class="warning">
        복사 전 확인이 끝나지 않아 새 복사를 차단합니다. 상태를 다시 확인한 뒤 계획을 실행하세요.
      </p>
    {/if}
    {#if report.capacity}
      {#if report.capacity.can_fit === true}
        <p class="capacity-ok">
          ...
          {#if report.capacity.snapshot.remaining_bytes !== null}
            · 남은 공간 {fmtBytes(report.capacity.snapshot.remaining_bytes)}
          {:else}
            · 저장 공간 제한 없음
          {/if}
        </p>
      {:else if report.capacity.can_fit === false}
        <p class="warning">
          클라우드 저장 공간이 부족합니다. 남은 공간을 확인한 뒤 파일 크기를 줄이거나 다른 계정을 선택하세요.
        </p>
      {:else}
        <p class="warning">
          클라우드 저장 공간을 확인하지 못했습니다. 연결 상태를 확인한 뒤 다시 시도하세요.
          ...
          {:else}
            OneDrive·Google Drive 연결을 완료한 뒤 다시 계획해야 복사할 수 있습니다.
          {/if}
          iCloud는 macOS 계정 상태를 확인한 뒤 다시 계획해야 복사할 수 있습니다.
        </p>
      {/if}
    {/if}
    {#if report.exact_duplicates.candidate_count > 0}
      <p class="warning">
        같은 파일로 보이는 후보 {report.exact_duplicates.candidate_count.toLocaleString()}개 ·
        {report.exact_duplicates.cluster_count.toLocaleString()}개 묶음 ·
        중복으로 보이는 공간 {fmtBytes(report.exact_duplicates.redundant_bytes)}.
        같은 파일 후보는 자동으로 복사하지 않으므로, 대표로 보관할 파일을 직접 확인한 뒤 진행하십시오.
      </p>
    {/if}
    <p class="warning">
      파일 안의 정보와 파일 날짜를 확인해 정리 순서를 제안합니다. 후보와 저장 위치를 확인하고 승인해야 복사할 수 있습니다. 이미 있는 클라우드 파일은 같은 내용인지 확인한 뒤 채택하며, 원본은 승인한 경우에만 휴지통으로 이동합니다. 휴지통은 비우지 않습니다.
    </p>
    {#if copied}
      <div class="receipt">
        ...
        {#if copied.receipt.provider === "google-drive"}
          <div class="provider-auth">
            <label>
              Google Drive 파일 확인 ID (선택)
              <input type="text" bind:value={objectId} autocomplete="off" disabled={attesting} />
            </label>
          </div>
          <p class="muted">파일 확인 ID를 입력하면 저장 위치와 내용이 복사한 파일과 일치하는지 다시 확인합니다. 확인할 수 없는 파일은 원본을 정리하지 않습니다.</p>
          <p class="muted">연결 정보는 이 기기에 안전하게 보관하며 화면에 표시하지 않습니다.</p>
        {:else if copied.receipt.provider === "onedrive"}
          <p class="muted">OneDrive 연결로 저장 위치와 내용을 다시 확인합니다. 확인할 수 없는 파일은 원본을 정리하지 않습니다.</p>
        {/if}
        <button
          onclick={attestCopy}
          disabled={attesting}
        >
          {attesting ? "클라우드 파일 확인 중…" : "클라우드 파일 확인"}
        </button>
        {#if attestation}
          <p class:warning={attestation.goal_state !== "eviction-ready"} class:safe={attestation.goal_state === "eviction-ready"}>
            클라우드 파일 확인: {attestation.goal_state === "eviction-ready" ? "완료" : "추가 확인 필요"} ·
            {syncStateLabel(attestation.evidence.sync_state)}
          </p>
          {#if attestation.assessment.state === "overdue"}
            <p class="warning">
              클라우드 파일 확인이 {Math.floor(attestation.assessment.pending_age_ms / 3_600_000)}시간째 완료되지 않았습니다. 원본은 계속 보존하며 상태를 다시 확인하세요.
            </p>
          {:else if attestation.assessment.state === "pending"}
            <p class="muted">
              클라우드 파일 확인 대기 {Math.floor(attestation.assessment.pending_age_ms / 60_000)}분. 완료 전에는 원본을 제거하지 않습니다.
            </p>
          {/if}
          {#if attestation.permit}
            {#if eviction}
              <p class="safe">원본을 휴지통으로 이동했습니다. 클라우드 파일은 유지되며 휴지통은 비우지 않았습니다.</p>
              <p class="muted">필요하면 휴지통에서 원본을 복원할 수 있습니다.</p>
            {:else}
              <p class="safe">클라우드 파일 확인 완료. 원본 정리를 진행할 수 있지만 파일은 아직 그대로 보존됩니다.</p>
              <div class="eviction-controls">
                <p class="warning">
                  아래 확인 문구를 직접 입력하고 이 파일만 휴지통으로 옮기는 사유를 남겨야 합니다. 실행 직전에 클라우드 상태를 다시 확인하며, 달라지면 중단합니다.
                </p>
                <div class="context">입력할 확인 문구: {copied.receipt.receipt_id}</div>
                  <label>
                    확인 문구 입력
                  <input
                ...
                <label>
                  ...
                  <textarea
                    ...
                    maxlength="1000"
                    disabled={evicting}
                    placeholder="예: 클라우드 파일 확인 후 이 파일의 원본만 휴지통으로 이동"
                  ></textarea>
                </label>
                <button onclick={evictVerifiedSource} disabled={!sourceEvictionReady()}>
                  {evicting ? "상태 재확인 후 이동 중…" : "다시 확인하고 원본을 휴지통으로 이동"}
                </button>
              </div>
            {/if}
          {:else}
            <p class="warning">아직 원본을 이동할 수 없습니다. 표시된 확인 항목을 해결한 뒤 다시 시도하세요.</p>
          {/if}
          <p class="muted">클라우드 파일 확인 결과를 저장했습니다.</p>
        {/if}
      </div>
    {/if}
        ...
        {#each reviewPageData.items as candidate (candidate.metadata_fingerprint)}
          <li class:blocked={candidate.blocked_reason !== null} class:adoptable={adoptEligible(candidate)}>
            <div class="line">
              ...
              <span>생산 정보: {productionSourceLabel(candidate.production_time_source)} ({confidenceLabel(candidate.production_time_confidence)})</span>
            ...
            {#if candidate.content_title}
              <div class="metadata">파일 제목: {candidate.content_title}</div>
            {/if}
            ...
            {#if candidate.content_context.length > 0}
              <div class="metadata">파일 맥락: {candidate.content_context.join(" · ")}</div>
            {/if}
            ...
            {#if candidate.dataset_profile}
              <div class="dataset-profile">
                <strong>
                  데이터 파일 정보: {candidate.dataset_profile.format.toUpperCase()} ·
                ...
                {/if}
                <div class="metadata">
                  {candidate.dataset_profile.profile_complete ? "파일 구조 확인 완료" : "파일 구조 확인 불완전·검토 필요"}
                ...
                {#if candidate.dataset_profile.quality_warnings.length > 0}
                  <div class="context">데이터 확인 경고: {candidate.dataset_profile.quality_warnings.join(", ")}</div>
                {/if}
            ...
            <div class="context">
              파일 종류: {sourceContextLabel(candidate.source_context)} · 대상 계정: {accountScopeLabel(candidate.destination_account_scope)}
            </div>
            <details class="lineage">
              <summary>파일 관계 확인</summary>
              <ol>
                <li>
                  원본 파일 · {sourceContextLabel(candidate.source_context)}
                </li>
                <li>
                  생산 시각 · {productionSourceLabel(candidate.production_time_source)} · 확인 수준 {confidenceLabel(candidate.production_time_confidence)}
                </li>
                <li>보관 파일 · {candidate.kind} · {fmtBytes(candidate.bytes)}</li>
                <li>클라우드 파일 · {cloudProviderLabel(candidate.provider)} · {accountScopeLabel(candidate.destination_account_scope)} · {candidate.dst}</li>
              </ol>
              <p class="context">
                파일 확인 → 업로드 확인 → 원본 이동 확인 순서로 진행됩니다.
                {candidate.blocked_reason
                  ? ` 현재 확인 필요: ${customerDecisionReasonLabel(candidate.blocked_reason)}.`
                  : " 복사 확인 코드가 생성되면 다음 단계를 진행할 수 있습니다."}
              </p>
            </details>
            {#if candidate.requires_review}
              <div class="review-controls">
                {#if matchingReviewDecision(candidate)?.disposition === "approved"}
                  <strong class="approved">현재 파일 정보 확인 승인됨</strong>
                ...
                {#if matchingReviewDecision(candidate)}
                  <span class="context">
                    확인 사유: {matchingReviewDecision(candidate)?.rationale ?? "이전 확인"}
                  </span>
                {/if}
                <label class="review-rationale">
                  새 승인·보류 사유 (민감한 셀 값이나 문서 본문은 입력하지 마세요)
                  <textarea
                    maxlength="1000"
                ...
                <button
                  ...
                  disabled={reviewingFingerprint !== ""
                    ...
                    || (organizationTenantAuthorityRequired(candidate)
                      && !(reviewTenantAuthorities[candidate.metadata_fingerprint] ?? false))}
                >
                  {reviewingFingerprint === candidate.metadata_fingerprint ? "기록 중…" : "파일 정보 검토 승인"}
                </button>
                <button
            ...
            {#if copyEligible(candidate) || providerApiCopyEligible(candidate)}
              ...
              <div class="copy-approval">
                <div class="context">현재 파일·출발지·목적지에 맞는 확인 문구를 정확히 입력해야 합니다.</div>
                  ...
                  <button
                    ...
                    disabled={copyingFingerprint !== ""
                      ...
                      || (copyConfirmations[candidate.metadata_fingerprint] ?? "").trim()
                        !== copyApprovalPhrase}
                >
                  {copyingFingerprint === candidate.metadata_fingerprint ? "클라우드 파일 확인 중…" : "원본을 유지하고 클라우드에 복사"}
                </button>
                {/if}
                {#if providerApiCopyEligible(candidate)}
                  <p class="warning">클라우드 동기화가 지연되어 업로드 연결로 직접 전송합니다. 원본은 유지되며 업로드 확인 후 다음 단계를 진행할 수 있습니다.</p>
                  <button
                    class="copy"
                    ...
                    disabled={copyingFingerprint !== ""
                      ...
                      || (copyConfirmations[candidate.metadata_fingerprint] ?? "").trim()
                        !== copyApprovalPhrase}
                  >
                    {copyingFingerprint === candidate.metadata_fingerprint ? "클라우드에 업로드 중…" : "업로드 연결로 클라우드에 복사"}
                  </button>
                {/if}
              </div>
            {/if}
            {#if adoptEligible(candidate)}
              ...
              <div class="copy-approval">
                <div class="context">기존 목적지 파일의 내용이 같은지 확인한 뒤 별도 승인을 진행하세요.</div>
                ...
                <button
                  ...
                  disabled={copyingFingerprint !== ""
                    ...
                    || (copyConfirmations[candidate.metadata_fingerprint] ?? "").trim()
                      !== adoptApprovalPhrase}
                >
                  {copyingFingerprint === candidate.metadata_fingerprint ? "기존 파일 확인 중…" : "기존 클라우드 파일 확인·사용"}
                </button>
              </div>
            {/if}
            <details>
              ...
              <ul class="evidence">
                {#each candidate.metadata_evidence as evidence}
                  <li>{evidence.field}: {evidence.value} · {productionSourceLabel(evidence.source)} · {confidenceLabel(evidence.confidence)}</li>
                {/each}
              </ul>
            </details>
            {#if candidate.review_reasons.length > 0}
              <div class="context">확인 필요: {candidate.review_reasons.map(customerDecisionReasonLabel).join(", ")}</div>
            {/if}
          </li>
        {/each}
      </ul>
