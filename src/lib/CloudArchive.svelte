<script lang="ts">
  ...
  function icloudBlockerLabel(blocker: string): string {
    const labels: Record<string, string> = {
      ...
      "icloud-file-provider-no-progress": "iCloud 파일 처리가 진행되지 않음",
      ...
      "icloud-file-provider-indexing-pending": "iCloud 파일 확인 대기 항목이 있음",
  ...
  function providerGlobalSyncBlockerLabel(blocker: string): string {
    const labels: Record<string, string> = {
      ...
      "provider-global-sync-filename-too-long": "파일 이름 제한 오류가 있음",
  ...
  function icloudStatusDetails(): string {
    if (!icloudHealth) return icloudHealthError || "iCloud 상태를 확인하는 중입니다.";
    ...
    return blockers.length > 0
      ? `새 복사 차단: ${blockers.join(", ")} · 파일 확인 대기 ${pending.toLocaleString()}개`
      : "iCloud 전체 동기화 대기열이 비어 있습니다. 개별 파일 상태를 확인하십시오.";
  }

  function providerGlobalStatusDetails(): string {
    if (!providerGlobalSync) return "클라우드 전체 동기화 상태를 확인하지 못했습니다. 잠시 후 다시 시도하십시오.";
    ...
    return blockers.length > 0
      ? `새 복사 차단: ${blockers.join(", ")} · 파일 확인 대기 ${pending.toLocaleString()}개`
      : "클라우드 전체 동기화 대기열이 비어 있습니다. 개별 파일 상태를 확인하십시오.";
  }
  ...
  function cloudProviderLabel(provider: string | null | undefined): string {
    return {
      icloud: "iCloud",
      onedrive: "OneDrive",
  ...
  function candidateKindLabel(kind: string | null | undefined): string {
    return {
      document: "문서",
      image: "이미지",
      audio: "오디오",
  ...
  function transferStatusLabel(status: string | null | undefined, state: string | null | undefined): string {
    if (status === "blocked" || state === "pending-provider-sync") return "업로드 확인 대기";
    if (status === "completed" || state === "provider-sync-confirmed" || state === "eviction-ready" || state === "source-evicted") return "업로드 확인 완료";
    return "상태 확인 필요";
  }

  function rootIssueLabel(issue: api.CloudRootDiscoveryIssue): string {
    if (issue.reason.includes("permission")) return "접근 권한 확인 필요";
    if (issue.reason.includes("not-found")) return "클라우드 위치를 찾지 못함";
    return "클라우드 위치를 확인하지 못함";
  }
</script>
...
<section>
  ...
  <p class="muted">
    iCloud Drive·OneDrive·Google Drive의 로컬 위치를 확인하고, 파일 정보를 바탕으로 생산 시점과 원래 위치를 보존하는 이동 계획만 만듭니다.
  </p>

  {#if roots.length === 0}
    <p class="warning">탐지된 클라우드 위치가 없습니다. 클라우드 앱이 연결되어 있는지 확인한 뒤 새로고침하십시오.</p>
  {:else}
    <div class="controls">
      <label>
        대상
      ...
      <span class="muted">화면이 열려 있는 동안 클라우드에 쓰거나 원본을 삭제하지 않고 상태만 확인합니다. 처리가 지연되면 자동 확인 간격이 최대 5분으로 늘어납니다.</span>
    </div>
    ...
    {#if reconciliation}
      <div class="receipt-reconciliation" aria-live="polite">
        ...
        {#if reconciliation.entries.length === 0}
          <p class="muted">저장된 작업 결과가 없습니다.</p>
        {:else}
          {#each reconciliation.entries as entry}
            <p class:warning={entry.error !== null || entry.blockers.length > 0}>
              ...
              {#if entry.blockers.length > 0} · 추가 확인 필요{/if}
            </p>
          {/each}
        {/if}
        <p class="muted">이 작업은 상태만 새로 확인하며 클라우드에 쓰거나 원본을 삭제하지 않습니다.</p>
      </div>
    {/if}
    ...
    {#if icloudHealth}
      <div class="receipt-reconciliation" aria-live="polite">
        ...
        <span class="context">
          ...
          업로드 대기 {icloudHealth.upload_queue.blocked_on_sync_up_count}개 ·
          오류 {icloudHealth.upload_queue.item_error_count}개
          {#if icloudHealth.file_provider_activity}
            · 진행되지 않은 파일 처리 {icloudHealth.file_provider_activity.no_progress_fetch_count}개 / {icloudHealth.file_provider_activity.no_progress_create_count}개 ·
            파일 준비 실패 {icloudHealth.file_provider_activity.materialization_failure_count}개 / 준비 항목 없음 {icloudHealth.file_provider_activity.staged_item_missing_count}개 ·
            동기화 제외(파일명) {icloudHealth.file_provider_activity.sync_excluded_filename_count}개 / 위치 {icloudHealth.file_provider_activity.sync_excluded_root_count}개 ·
            파일 확인 대기 {icloudHealth.file_provider_activity.pending_indexable_count ?? 0}개 ·
            진행 중 업로드 {icloudHealth.file_provider_activity.active_upload_count}개 / 다운로드 {icloudHealth.file_provider_activity.active_download_count}개
        ...
        {#if icloudHealthBlockedSinceMs > 0}
          <p class="muted">
            같은 상태 지속: {duration(Math.max(0, icloudHealth.observed_at_ms - icloudHealthBlockedSinceMs))}
          </p>
        {/if}
        {#if hasIcloudHealthEvidencePersistenceFailure(icloudHealth.notices)}
          <p class="warning">
            iCloud 동기화 요약을 저장하지 못했습니다. 이번 확인 결과만 표시하며 이전 상태와 비교하지 않습니다.
            저장 공간과 권한을 확인한 뒤 다시 시도하십시오.
          </p>
        {/if}
        ...
        {#if icloudHealth.new_copy_admission_blockers.length > 0}
          ...
          {#if icloudHealth.file_provider_activity && (icloudHealth.file_provider_activity.no_progress_fetch_count > 0 || icloudHealth.file_provider_activity.no_progress_create_count > 0)}
            <p class="warning">
              ...
              클라우드 상태가 정상으로 확인된 뒤 DiskSage에서 새 계획을 다시 실행하십시오.
            </p>
          {/if}
          {#if icloudHealth.file_provider_activity && (icloudHealth.file_provider_activity.materialization_failure_count > 0 || icloudHealth.file_provider_activity.staged_item_missing_count > 0)}
            <p class="warning">
              클라우드 파일 준비가 완료되지 않았습니다. 현재 복사는 완료로 간주하지 않으며,
              상태가 정상화될 때까지 새 복사와 원본 정리를 막습니다. 잠시 후 상태를 다시 확인하십시오.
            </p>
          {/if}
          ...
          {#if icloudHealth.file_provider_activity?.timed_out}
            <p class="warning">
              ...
              DiskSage에서 상태를 다시 확인한 뒤 정상으로 확인될 때만 새 복사를 시작하십시오.
            </p>
          {/if}
          {#if icloudHealth.file_provider_activity && (icloudHealth.file_provider_activity.active_upload_count > 0 || icloudHealth.file_provider_activity.active_download_count > 0)}
            <p class="warning">
              iCloud에 기존 전송이 진행 중입니다. 기존 업로드와 다운로드가 끝나고 상태가 정상으로 확인될 때까지 Finder 복사와 원본 정리를 진행하지 않습니다.
            </p>
          {/if}
          {#if (icloudHealth.file_provider_activity?.pending_indexable_count ?? 0) > 0}
            <p class="warning">
              iCloud 파일 확인 대기 항목이 {icloudHealth.file_provider_activity?.pending_indexable_count}개 있습니다.
              Finder의 “복사 준비 중” 단계가 이 대기열을 기다릴 수 있으므로, 대기와 기존 전송이 해소되기 전에는 복사를 완료로 간주하지 않습니다.
            </p>
          {/if}
          {#if icloudHealthBlockedSinceMs > 0 && icloudHealth.observed_at_ms - icloudHealthBlockedSinceMs >= PROVIDER_STALL_WARNING_MS}
            <p class="warning">
              ...
              iCloud 상태가 정상으로 확인될 때까지 새 복사와 원본 정리를 시작하지 마십시오.
            </p>
          {/if}
        {:else}
          <p class="capacity-ok">iCloud 전체 업로드 대기열이 비어 있습니다. 개별 파일 상태를 확인한 뒤 진행하십시오.</p>
        {/if}
        ...
        {/if}
        <p class="muted">읽기 전용 상태 확인이며, 클라우드에 쓰거나 원본을 삭제하지 않습니다.</p>
      </div>
    {/if}
    ...
    {#if selectedRootDetails()?.provider !== "icloud" && checkingProviderGlobalSync && !providerGlobalSync && !providerGlobalSyncError}
      <ProviderStatusCard
        provider={cloudProviderLabel(selectedRootDetails()?.provider)}
        state={providerStatusState(false, false, 0, 0)}
        details="클라우드 전체 동기화 상태를 확인하는 중입니다. 확인이 끝날 때까지 새 복사와 원본 정리를 시작하지 마십시오."
    ...
    {#if providerGlobalSync}
      <ProviderStatusCard
        provider={cloudProviderLabel(providerGlobalSync.provider)}
        state={providerStatusState(
          true,
      ...
      <div class="receipt-reconciliation" aria-live="polite">
        ...
        <span class="context">
          ...
          {#if providerGlobalSyncBlockedSinceMs > 0}
            · 같은 상태 지속 {duration(Math.max(0, providerGlobalSyncObservedAtMs - providerGlobalSyncBlockedSinceMs))}
          {/if}
        </span>
        {#if finderCopyCancelStatus}<p class="muted">{finderCopyCancelStatus}</p>{/if}
        {#if providerGlobalSync.blockers.length > 0}
          ...
          {#if providerGlobalSyncBlockedSinceMs > 0 && providerGlobalSyncObservedAtMs - providerGlobalSyncBlockedSinceMs >= PROVIDER_STALL_WARNING_MS}
            <p class="warning">
              동일한 클라우드 차단 상태가 15분 이상 지속되었습니다. Finder에 남은 복사 대기를 취소하고,
              클라우드 앱을 재기동한 뒤 상태가 정상으로 확인될 때까지 새 복사와 원본 정리를 시작하지 마십시오.
            </p>
          {/if}
          {#if selectedRootDetails()?.provider !== "icloud"}
          ...
          {/if}
        {:else}
          <p class="capacity-ok">클라우드 전체 동기화 대기열이 비어 있습니다. 개별 파일 상태를 확인한 뒤 진행하십시오.</p>
        {/if}
        {#if providerRecovery}
          <p class:warning={providerRecovery.blockers.length > 0} class="muted">
            클라우드 앱 재기동 요청 완료 · 재확인
    ...
    {#if providerGlobalSyncError}
      <p class="error" role="alert">클라우드 전체 동기화 상태 확인에 실패했습니다. 다시 시도하십시오.</p>
      <p class="warning">
        클라우드 전체 상태를 확인하지 못했습니다. Finder에 남은 복사 대기를 취소하고,
        클라우드 앱이 정상으로 확인될 때까지 새 복사와 원본 정리를 시작하지 마십시오.
      </p>
    {/if}
    ...
    {#if rootIssues.length > 0}
      <p class="warning">
        클라우드 위치 확인 문제 {rootIssues.length}건: {rootIssues.map(rootIssueLabel).join(", ")}
      </p>
    {/if}
    {#if selectedRootDetails()?.provider === "icloud"}
      <div class="oauth-panel">
        <strong>macOS iCloud 계정 저장 공간</strong>
        ...
        {:else}
          <p class="muted">별도 로그인 없이 macOS의 읽기 전용 iCloud 계정 상태를 사용합니다.</p>
        {/if}
        {#key selectedRoot}
    ...
    {:else if selectedRootDetails()}
      <div class="oauth-panel">
        {#if connectionForSelectedRoot()}
          ...
          {:else}
            <p class="muted">
              연결 정보만 확인했습니다. 재시작 후 실제 클라우드 연결과 저장 공간을 다시 확인하십시오.
            </p>
          {/if}
        {:else}
          <label>
            {selectedRootDetails()?.provider === "onedrive" ? "Microsoft 연결 ID" : "Google 연결 ID"}
            <input
              class="client-id"
              type="text"
          ...
          <label>
            ...
            클라우드 앱이 지연될 때 직접 업로드할 권한도 요청
          </label>
          <button onclick={connectProvider} disabled={connecting || !oauthClientId.trim()}>
            {connecting ? "브라우저 동의 대기 중…" : "시스템 브라우저로 클라우드 연결"}
          </button>
          <p class="muted">
            연결 동의는 시스템 브라우저에서 진행하며 로그인 정보는 앱 화면에 표시하거나 저장하지 않습니다.
          </p>
          {#if selectedRootDetails()?.provider === "onedrive"}
            <p class="muted">Microsoft 연결 앱을 준비한 뒤 안내되는 계정 동의를 완료하십시오. 동의하지 않으면 읽기 전용 확인만 사용합니다.</p>
          {/if}
          {#if selectedRootDetails()?.provider === "google-drive"}
            <p class="warning">Google 연결은 Desktop app 유형이어야 합니다. 직접 업로드를 선택하면 Drive 파일 쓰기 권한 동의가 필요합니다. 동의하지 않으면 읽기 전용 확인만 사용합니다.</p>
          {/if}
        {/if}
      </div>
    {/if}
  ...
  {#if report}
    ...
    {#if hasProviderAdmissionBlocker(report.notices)}
        <p class="warning">
          클라우드 전체 동기화 상태가 완전하지 않거나 전송 중입니다. 새 복사는 비활성화되며,
          상태가 정상으로 확인된 뒤 다시 계획하십시오. 기존 파일 확인은 별도 경로로 진행할 수 있습니다.
        </p>
    {/if}
    ...
    {#if report.local_volume}
      ...
      {#if hasLocalEvidencePersistenceFailure(report.notices)}
        <p class="warning">
          디스크 용량 확인을 저장하지 못했습니다. 이번 계획은 실행할 수 있지만 이전 상태와 비교할 수 없습니다. 저장 공간과 권한을 확인한 뒤 다시 시도하십시오.
        </p>
      {/if}
      {#if report.candidates.some(nativeCopyHeadroomBlocked)}
        <p class="warning">
          클라우드 복사는 후보 크기와 {fmtBytes(api.LOCAL_COPY_RESERVE_BYTES)} 여유공간을 함께 확보해야 합니다.
          현재 여유공간이 부족한 후보는 버튼을 비활성화합니다. 연결된 클라우드 서비스 업로드는 별도 경로입니다.
        </p>
      {/if}
    {/if}
    {#if hasRuntimeEvidencePersistenceFailure(report.notices)}
      <p class="warning">
        클라우드 상태 이력을 저장하지 못했습니다. 이번 확인 결과만 사용하며, 저장 공간과 권한을 확인한 뒤 다시 시도하십시오.
      </p>
    {/if}
    {#if hasIcloudHealthEvidencePersistenceFailure(report.notices)}
      <p class="warning">
        iCloud 동기화 요약을 저장하지 못했습니다. 이번 계획은 표시되지만 이전 상태와 비교할 수 없습니다. 저장 공간과 권한을 확인한 뒤 다시 시도하십시오.
        상태가 정상으로 관찰된 뒤 다시 계획하십시오.
      </p>
    {/if}
    {#if report.pre_copy_evidence && !report.pre_copy_evidence.complete}
      <p class="warning">
        복사 전 상태 확인이 완성되지 않아 새 복사를 차단합니다. 다음 항목을 확인하십시오:
    ...
    {#if report.capacity}
      ...
      {:else}
        <p class="warning">
          ...
          {:else}
            OneDrive·Google Drive 연결을 설정한 뒤 다시 계획하십시오.
          {/if}
          iCloud 계정 상태를 확인한 뒤 다시 계획하십시오.
        </p>
      {/if}
    {/if}
    {#if report.exact_duplicates.candidate_count > 0}
      <p class="warning">
        ...
        동일 크기 후보만 로컬 SHA-256·BLAKE3로 확인했으며, 대표 파일을 선택하기 전에는 자동 복사하지 않습니다.
        정본 추천은 파일의 생산일·신뢰도·내장 정보를 먼저 비교하고, 다운로드·압축해제
    ...
    {/if}
    <p class="warning">
        파일 정보와 변경 시각을 확인해 정리 순서를 제안합니다. 제안된 항목은 확인 문구와 사유를 입력한 뒤에만 복사할 수 있습니다. 원본은 업로드 확인 후 별도로 승인할 때까지 보존됩니다.
    </p>
    {#if copied}
      <div class="receipt">
        ...
        {#if attestation}
          <p class:warning={attestation.goal_state !== "eviction-ready"} class:safe={attestation.goal_state === "eviction-ready"}>
            업로드 확인: {transferStatusLabel(attestation.goal_status, attestation.goal_state)} ·
            {syncStateLabel(attestation.evidence.sync_state)}
          </p>
          {#if attestation.assessment.state === "overdue"}
            <p class="warning">
              클라우드 업로드 확인이 {Math.floor(attestation.assessment.pending_age_ms / 3_600_000)}시간째 완료되지 않았습니다. 원본은 계속 보존하며 클라우드 상태를 확인하십시오.
            </p>
          {:else if attestation.assessment.state === "pending"}
            <p class="muted">
              클라우드 업로드 확인 대기 {Math.floor(attestation.assessment.pending_age_ms / 60_000)}분입니다. 완료 전에는 원본을 제거하지 않습니다.
            </p>
          {/if}
          {#if attestation.permit}
            ...
            {:else}
              <p class="safe">업로드 상태와 복사 내용을 확인했습니다. 원본은 아직 그대로 보존됩니다.</p>
              <div class="eviction-controls">
                ...
                <label>
                  ...
                  <textarea
                    ...
                    maxlength="1000"
                    disabled={evicting}
                    placeholder="예: 클라우드 업로드와 파일 확인이 끝난 원본만 휴지통으로 이동"
                  ></textarea>
                </label>
                <button onclick={evictVerifiedSource} disabled={!sourceEvictionReady()}>
                  {evicting ? "상태 재확인 후 이동 중…" : "검증을 다시 수행하고 원본을 휴지통으로 이동"}
                </button>
              </div>
            {/if}
          {:else}
            <p class="warning">아직 원본을 이동할 수 없습니다. 파일 상태를 다시 확인하십시오.</p>
          {/if}
        {/if}
      </div>
    {/if}
    ...
    {:else}
      <div class="review-queue" aria-label="클라우드 파일 정보 검토 큐">
        ...
        {#each reviewPageData.items as candidate (candidate.metadata_fingerprint)}
          <li class:blocked={candidate.blocked_reason !== null} class:adoptable={adoptEligible(candidate)}>
            <div class="line">
              ...
              <span>확인 정보: {productionSourceLabel(candidate.production_time_source)} ({candidate.production_time_confidence === "high" ? "높음" : "추가 확인 필요"})</span>
            ...
            <details class="lineage">
              <summary>파일 정리 흐름</summary>
              <ol>
                <li>
                  원본 파일 · {candidate.source_context}
                </li>
                <li>
                  생산 시각 확인 · {productionSourceLabel(candidate.production_time_source)} · 신뢰도 {candidate.production_time_confidence === "high" ? "높음" : "추가 확인 필요"}
                </li>
                <li>보관 파일 · {candidateKindLabel(candidate.kind)} · {fmtBytes(candidate.bytes)}</li>
                <li>클라우드 위치 · {cloudProviderLabel(candidate.provider)} · {accountScopeLabel(candidate.destination_account_scope)} · {candidate.dst}</li>
              </ol>
              <p class="context">
                파일 확인 → 클라우드 업로드 확인 → 원본 이동 승인 순서로 진행됩니다.
                {candidate.blocked_reason
                  ? ` 현재 관계 차단: ${customerDecisionReasonLabel(candidate.blocked_reason)}.`
                  : " 아직 업로드 확인과 원본 이동 허가가 확정되지 않았습니다."}
              </p>
            </details>
            {#if candidate.requires_review}
              <div class="review-controls">
                ...
                {#if matchingReviewDecision(candidate)}
                  <span class="context">
                    검토자: {matchingReviewDecision(candidate)?.reviewed_by ?? "로컬 검토자"} ·
                    사유: {matchingReviewDecision(candidate)?.rationale ?? "사유 없음"}
                  </span>
                {/if}
            ...
            {#if copyEligible(candidate) || providerApiCopyEligible(candidate)}
              ...
              <div class="copy-approval">
                <div class="context">현재 파일·출발지·목적지에 맞는 문구를 정확히 입력해야 합니다.</div>
                ...
                {#if providerApiCopyEligible(candidate)}
                  <p class="warning">클라우드 전체 동기화가 막혀 있어 연결된 서비스로 직접 업로드합니다. 원본은 유지되고 업로드 확인이 필요합니다.</p>
                  <button
                    class="copy"
            ...
            {/if}
            <details>
              <summary>파일 정보 확인 {candidate.metadata_evidence.length}건</summary>
              <ul class="evidence">
