...
describe("verdictBadge", () => {
  it("keeps safety guidance distinct without presenting safe as deletion authorization", () => {
    const safe = verdictBadge("safe");
    expect(safe.label).toBe("낮은 위험");
    expect(safe.title).toContain("확인하세요");
  ...
  it("falls back to the unrated badge for unknown input", () => {
    ...
    expect(verdictBadge("bogus" as any).title).toContain("직접 확인하세요");
  });
});
